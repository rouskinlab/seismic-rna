...
def run(
    *,
    ...
    start_files = {
        start: list(path.find_files(Path(files), [path.ConnectTableSeg]))
    ...
    multi_starts = {
        file: sorted(starts) for file, starts in file_starts.items() if len(starts) > 1
    }
    if multi_starts:
        logger.warning(
...
params = [opt_ct_pos_5, opt_inplace, opt_out_dir, opt_force, opt_num_cpus]
