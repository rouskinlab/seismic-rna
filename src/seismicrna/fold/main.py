...
def resolve_fold_backend(probe: str, fold_backend: str) -> str:
    if fold_backend == FOLD_BACKEND_AUTO:
        return (FOLD_BACKEND_RNASTRUCTURE if probe == PROBE_DMS
...
def fold_region(rna: RNAFoldProfile, *,
    ...
    if need_write(report_file, force):
        ...
        eddy_prior_paired_file_checksum = (
            calc_sha512_path(eddy_prior_paired_file)
            if eddy_prior_paired_file else ""
        )
        eddy_prior_unpaired_file_checksum = (
            calc_sha512_path(eddy_prior_unpaired_file)
            if eddy_prior_unpaired_file else ""
        )
        ended = datetime.now()
