...
# SAM filters
EXCLUDE_FLAGS = (
    FLAG_UNMAP | FLAG_SECONDARY | FLAG_QCFAIL | FLAG_DUPLICATE | FLAG_SUPPLEMENTARY
)
...
def fastp_cmd(
    ...
    *,
    ...
    if fq_inp.phred_enc == 64:
        ...
        raise ValueError(
            "fastp requires a Phred encoding of +33 or +64, "
            f"but got +{fq_inp.phred_enc}"
        )
    if fq_inp.interleaved:
        args.append("--interleaved_in")
    for flag, value in zip(["-i", "-I"], fq_inp.paths.values(), strict=False):
        args.extend([flag, value])
    # Output files
    if fq_out is not None:
        for flag, value in zip(["-o", "-O"], fq_out.paths.values(), strict=False):
            args.extend([flag, value])
    else:
        # Output to stdout.
        args.append("--stdout")
...
run_fastp = ShellCommand("trimming base calls and filtering reads from", fastp_cmd)
...
def bowtie2_build_cmd(fasta: Path, prefix: Path, *, num_cpus: int = 1):
    ...
    args = [BOWTIE2_BUILD_CMD, "-q", "--threads", num_cpus, fasta, prefix]
    return args_to_cmd(args)


run_bowtie2_build = ShellCommand("building Bowtie 2 index for", bowtie2_build_cmd)
    ...
    *,
    ...
    args.extend(
        [
            ...
            "--ignore-quals",
            "--ma",
            MATCH_BONUS if bt2_local else "0",
            "--mp",
            MISMATCH_PENALTY,
            "--np",
            N_PENALTY,
            ...
            # Filtering
            "--score-min",
            (bt2_score_min_loc if bt2_local else bt2_score_min_e2e),
            "-I",
            bt2_i,
            "-X",
            bt2_x,
...
def xamgen_cmd(
    ...
    *,
    ...
    num_cpus_bowtie2 = max(num_cpus - 2, 1)
    if fastp:
        # Allocate half of the remaining processors to Fastp.
        num_cpus_fastp = max(num_cpus_bowtie2 // 2, 1)
        num_cpus_bowtie2 = max(num_cpus_bowtie2 - num_cpus_fastp, 1)
        if fq_inp.one_ref:
...
def flags_cmds(
    ...
    *,
    ...
    if not isinstance(xam_inp, Path):
        raise TypeError(
            f"Expected xam_inp to be a Path, but got {type(xam_inp).__name__}"
        )
    ...
    if len(flags_exc) != num_flags:
        raise ValueError(
            f"Numbers of flags to require ({num_flags}) and "
            f"exclude ({len(flags_exc)}) are different"
        )
    if num_flags == 0:
        # View the XAM file with no flags.
        return [view_xam_cmd(xam_inp, xam_out, num_cpus=num_cpus)]
    num_cpus_per_view = max(num_cpus // (num_flags + 1), 1)
    ...
    num_cpus_collate = max(num_cpus - num_cpus_per_view * num_flags, 1)
    return [
        cmds_to_subshell(view_xam_cmds),
        collate_xam_cmd(
            None, xam_out, tmp_pfx=tmp_pfx, fast=True, num_cpus=num_cpus_collate
        ),
    ]
...
def realign_cmd(
    ...
    **kwargs,
    ...
    if paired is None:
        paired = xam_paired(run_flagstat(xam_inp, num_cpus=num_cpus))
    # Reserve processors for the flags commands, conversion to FASTQ,
    # and filtering.
    num_flags = 1 if isinstance(flags_req, int) else len(flags_req := list(flags_req))
    num_cpus_bowtie2 = max(num_cpus - max(num_flags, 1) - 2, 1)
    cmds = flags_cmds(
        xam_inp, None, tmp_pfx=tmp_pfx, flags_req=flags_req, flags_exc=flags_exc
    )
    ...
    cmds.append(
        bowtie2_cmd(
            None, None, paired=paired, fq_unal=None, num_cpus=num_cpus_bowtie2, **kwargs
        )
    )
...
run_realign = ShellCommand(
    "filtering, collating, realigning, and formatting", realign_cmd
)
...
def export_cmd(
    ...
    *,
    ...
    num_cpus_per_view = max((num_cpus - 1) // 2, 1)
    ...
    ref_step = view_xam_cmd(
        xam_in, None, sam=True, with_header=False, ref=ref, num_cpus=num_cpus_per_view
    )
    ...
    export_step = view_xam_cmd(
        None, xam_out, refs_file=ref_file, num_cpus=num_cpus_per_view
    )
    return cmds_to_pipe([merge_step, export_step])


run_export = ShellCommand("selecting reference and exporting", export_cmd)
