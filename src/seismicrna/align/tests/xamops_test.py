...
class TestFlagsCmds(ut.TestCase):
    ...
    def test_1_flag_bam(self):
        result = flags_cmds(self.XAM_INP, self.XAM_OUT, flags_req=16, flags_exc=128)
        expect = [f"samtools view -@ 0 -f 16 -F 128 -o {self.XAM_OUT} {self.XAM_INP}"]
        self.assertEqual(result, expect)

    def test_1_flag_none(self):
        result = flags_cmds(self.XAM_INP, None, flags_req=16, flags_exc=128)
        expect = [f"samtools view -@ 0 -f 16 -F 128 -u {self.XAM_INP}"]
        self.assertEqual(result, expect)

    def test_2_flags_bam(self):
        result = flags_cmds(
            self.XAM_INP, self.XAM_OUT, flags_req=[16, 32], flags_exc=[128, 256]
        )
        expect = [
    ...
    def test_2_flags_none(self):
        result = flags_cmds(
            self.XAM_INP, None, flags_req=[16, 32], flags_exc=[128, 256]
        )
        expect = [
            f"( samtools view -@ 0 -f 16 -F 128 -h {self.XAM_INP} "
            f"; samtools view -@ 0 -f 32 -F 256 {self.XAM_INP} )",
