...
def require_issubclass(
    ...
    if not issubclass(value, classes):
        ...
        raise error_type(
            f"{name} must be a subclass of {classes}, but got {repr(value)}"
        )


def require_isinstance(
    name: str,
    value: Any,
    classes: type | tuple[type | tuple[Any, ...], ...],
    error_type: Type[TypeError] = TypeError,
) -> None:
    ...
    if not isinstance(value, classes):
        ...
        raise error_type(
            f"{name} must be an instance of {classes}, "
...
def require_isin(
    ...
    if value not in values:
        ...
        if values_name:
            message = (
                f"{name} must be in {values_name}, "
                f"but got {name}={repr(value)} "
                f"and {values_name}={repr(values)}"
            )
        else:
            message = f"{name} must be in {repr(values)}, but got {repr(value)}"
...
def _require_compare(
    ...
    require_isin(
        ...
        comparison,
        comparison_signs,
        error_type=(ValueError if callable(comparison) else TypeError),
    )
    if not comparison(value, compare_value):
        ...
        if compare_name:
            message = (
                f"Must have {name} {sign} {compare_name}, "
                f"but got {name}={repr(value)} "
                f"and {compare_name}={repr(compare_value)}"
            )
        else:
            message = (
                f"Must have {name} {sign} {repr(compare_value)}, but got {repr(value)}"
            )
        raise error_type(message)
...
def require_equal(
    ...
    _require_compare(
        name, value, eq, other_name, "other", other_value, classes, error_type
    )
...
def require_atleast(
    ...
    _require_compare(
        name, value, ge, minimum_name, "minimum", minimum_value, classes, error_type
    )
...
def require_greater(
    ...
    """Require that value > other_value."""
    _require_compare(
        name, value, gt, other_name, "other", other_value, classes, error_type
    )
...
def require_atmost(
    ...
    _require_compare(
        name, value, le, maximum_name, "maximum", maximum_value, classes, error_type
    )
...
def require_less(
    ...
    """Require that value < other_value."""
    _require_compare(
        name, value, lt, other_name, "less", other_value, classes, error_type
    )
...
def require_fraction(
    ...
    require_between(name, value, 0.0, 1.0, classes=classes, error_type=error_type)
