...
def _count_end_coords(end5s: np.ndarray, end3s: np.ndarray):
    ...
    return pd.Series(counts, pd.MultiIndex.from_arrays(uniq_ends.T, names=END_COORDS))


def _count_end_coords_weights(
...
def count_end_coords(
    end5s: np.ndarray, end3s: np.ndarray, weights: pd.DataFrame | None = None
):
    """Count each pair of 5' and 3' end coordinates."""
...
def _calc_coverage(
    ...
    np.add.at(
        per_read, row_indices_5, base_count[end3_coords] - base_count[end5_coords]
    )
...
def calc_rels_per_pos(
    ...
    if read_weights is not None:
        ...
        if not isinstance(read_weights, array_type):
            raise TypeError(
                f"Expected read_weights to be {array_type}, "
        ...
        if slice_type is not int:
            raise TypeError(f"Expected num_reads to be {int}, but got {slice_type}")
        slice_indexes = dict()
        if array_type is not pd.Series:
            raise TypeError(
                f"Expected cover_per_pos to be {pd.Series}, but got {array_type}"
            )
    ...
    for pos_base in cov_indices:
        ...
        counts[MATCH].loc[pos_base] = (
            cover_per_pos.loc[pos_base] - num_reads_pos
        ).round(PRECISION)
        if np.atleast_1d(counts[MATCH].loc[pos_base]).min(initial=0) < 0:
            raise ValueError(
                "Number of matches must be ≥ 0, "
        ...
        counts[NOCOV].loc[pos_base] = (num_reads - cover_per_pos.loc[pos_base]).round(
            PRECISION
        )
        if np.atleast_1d(counts[NOCOV].loc[pos_base]).min(initial=0) < 0:
            raise ValueError(
                "Number of non-covered positions must be ≥ 0, "
...
def calc_reads_per_pos(
    ...
    for pos, base in pos_index:
        pos_reads = [
            pos_mut_reads
            for mut, pos_mut_reads in mutations.get(pos, dict()).items()
...
def calc_covered_reads_per_pos(
    pos_index: pd.Index,
    read_nums: np.ndarray,
    seg_end5s: np.ndarray,
    seg_end3s: np.ndarray,
...
def calc_count_per_pos(
    ...
    if array_type is pd.Series:
        ...
        raise TypeError(
            f"Expected cover_per_pos to be {pd.Series} or "
