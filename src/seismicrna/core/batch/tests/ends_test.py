...
from seismicrna.core.batch.ends import (
    BadSegmentEndsError,
    ...
    def test_valid(self):
        for num_reads in range(5):
            for num_segs in range(5):
                ...
                self.assertEqual(count_reads_segments(ends), (num_reads, num_segs))
        ...
        for r5, r3, rm in product(range(4), repeat=3):
            for s5, s3, sm in product(range(4), repeat=3):
                ...
                if r5 != r3:
                    ...
                    self.assertEqual(
                        match_reads_segments(end5s, end3s, None), end5s.shape
                    )
                    if r5 != rm:
                        self.assertRaisesRegex(
                            BadSegmentEndsError,
                        ...
                        self.assertEqual(
                            match_reads_segments(end5s, end3s, mask), end5s.shape
                        )


class TestFindReadEnd5s(ut.TestCase):
    def test_0_segments(self):
        end5s = np.array([[], []], dtype=np.uint8)
    ...
    def test_1_segment_none_masked(self):
        end5s = np.array([[255], [2]], dtype=np.uint8)
        original = np.copy(end5s)
    ...
    def test_2_segments_none_masked(self):
        end5s = np.array([[5, 4], [2, 9]], dtype=np.uint8)
        original = np.copy(end5s)
        expect = np.array([4, 2])
        self.assertTrue(np.array_equal(find_read_end5s(end5s, None), expect))
        self.assertTrue(np.array_equal(end5s, original))
...
class TestFindReadEnd3s(ut.TestCase):
    ...
    def test_1_segment_none_masked(self):
        end3s = np.array([[5], [2]], dtype=np.uint8)
        original = np.copy(end3s)
        expect = np.array([5, 2])
        self.assertTrue(np.array_equal(find_read_end3s(end3s, None), expect))
        self.assertTrue(np.array_equal(end3s, original))
    ...
    def test_2_segments_none_masked(self):
        end3s = np.array([[5, 4], [2, 9]], dtype=np.uint8)
        original = np.copy(end3s)
        expect = np.array([5, 9])
        self.assertTrue(np.array_equal(find_read_end3s(end3s, None), expect))
        self.assertTrue(np.array_equal(end3s, original))
...
class TestMergeReadEnds(ut.TestCase):
    def test_valid(self):
        for num_reads in range(5):
            ...
            self.assertTrue(np.array_equal(merge_read_ends(end5s, end3s), expect))
...
class TestSortSegmentEnds(ut.TestCase):
    def test_0_segments(self):
        ...
        result_ends, result_contig5, result_contig3 = sort_segment_ends(
            end5s, end3s, None
        )
    ...
    def test_1_segment_mask(self):
        ...
        end5s = np.array([[1], [2]])
        end3s = np.array([[3], [0]])
        expect_ends = np.array([[0, 3], [0, 0]])
        expect_contig5 = np.array([[True, False], [False, False]])
        expect_contig3 = np.array([[False, True], [False, False]])
        result_ends, result_contig5, result_contig3 = sort_segment_ends(
            end5s, end3s, mask
        )
        self.assertTrue(np.array_equal(result_ends, expect_ends))
    ...
    def test_2_segments_nomask(self):
        end5s = np.array([[1, 4], [1, 3], [1, 2], [1, 1], [2, 1], [3, 1], [4, 1]])
        end3s = np.array([[2, 5], [2, 4], [2, 3], [2, 2], [3, 2], [4, 2], [5, 2]])
        expect_ends = np.array(
            [
                [0, 2, 3, 5],
                [0, 2, 2, 4],
    ...
    def test_n_segments_contig(self):
        for n in range(1, 10):
            ...
            expect_contig5 = np.array([True] + [False] * (2 * n - 1))[np.newaxis, :]
            expect_contig3 = np.array([False] * (2 * n - 1) + [True])[np.newaxis, :]
            expect_ends = np.concatenate([[0]] + [[i, i] for i in range(1, n)] + [[n]])[
                np.newaxis, :
            ]
            result_ends, result_contig5, result_contig3 = sort_segment_ends(
                ends, ends, None
...
class TestFindContiguousReads(ut.TestCase):
    def test_0_segments(self):
        for n_reads in range(10):
            ...
            self.assertTrue(
                np.array_equal(
                    find_contiguous_reads(end5s, end3s, None),
    ...
    def test_2_segments_nomask(self):
        ...
        self.assertTrue(
            np.array_equal(find_contiguous_reads(end5s, end3s, None), expect)
        )
