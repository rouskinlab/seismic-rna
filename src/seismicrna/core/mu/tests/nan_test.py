...
from seismicrna.core.mu.nan import (
    any_nan,
    no_nan,
    remove_nan,
    removes_nan,
...
class TestAnyNaN(ut.TestCase):
    ...
    def test_array2d(self):
        mus = np.array(
            [[0.0, 0.5], [0.1, np.nan], [np.nan, np.nan], [0.3, 0.8], [np.nan, 0.9]]
        )
    ...
    def test_series(self):
        mus = pd.Series([0.0, 0.1, np.nan, 0.3, 0.4], index=[1, 2, 6, 7, 8])
        expect = pd.Series([False, False, True, False, False], index=[1, 2, 6, 7, 8])
        a = any_nan(mus)
    ...
    def test_dataframe(self):
        ...
        expect = pd.Series([False, True, True, False, True], index=[1, 2, 6, 7, 8])
        a = any_nan(mus)
        self.assertIsInstance(a, pd.Series)
        self.assertTrue(a.equals(expect))
...
class TestNoNaN(ut.TestCase):
    ...
    def test_series(self):
        mus = pd.Series([0.0, 0.1, np.nan, 0.3, 0.4], index=[1, 2, 6, 7, 8])
        expect = pd.Series([True, True, False, True, True], index=[1, 2, 6, 7, 8])
        a = no_nan(mus)
        self.assertIsInstance(a, pd.Series)
        self.assertTrue(a.equals(expect))
    ...
    def test_dataframe(self):
        ...
        expect = pd.Series([True, False, False, True, False], index=[1, 2, 6, 7, 8])
        a = no_nan(mus)
        self.assertIsInstance(a, pd.Series)
        self.assertTrue(a.equals(expect))
...
class TestRemoveNaNs(ut.TestCase):
    ...
    def test_array2d(self):
        ...
        expect = np.array([[0.0, 0.5], [0.3, 0.8]])
        a = remove_nan(mus)
        self.assertIsInstance(a, np.ndarray)
        self.assertTrue(np.array_equal(a, expect))
    ...
    def test_series(self):
        mus = pd.Series([0.0, 0.1, np.nan, 0.3, 0.4], index=[1, 2, 6, 7, 8])
        expect = pd.Series([0.0, 0.1, 0.3, 0.4], index=[1, 2, 7, 8])
        a = remove_nan(mus)
        self.assertIsInstance(a, pd.Series)
        self.assertTrue(a.equals(expect))
    ...
    def test_dataframe(self):
        ...
        expect = pd.DataFrame(
            [[0.0, 0.5], [0.3, 0.8]], index=[1, 7], columns=["x", "y"]
        )
        a = remove_nan(mus)
...
class TestRemovesNaNs(ut.TestCase):
    def test_empty(self):
        self.assertRaisesRegex(
            ValueError, "requires at least 1 array, but got 0 arrays", removes_nan
        )
    ...
    def test_array_series_dataframe(self):
        ...
        s = pd.Series([0.5, 0.6, 0.7, np.nan, 0.9], index=[0, 1, 2, 3, 4])
        f = pd.DataFrame(
            [[0.0, 0.5], [0.1, 0.6], [np.nan, 0.7], [0.3, 0.8], [0.4, 0.9]],
            index=[5, 6, 7, 8, 9],
            columns=["x", "y"],
        )
        ...
        self.assertTrue(sa.equals(pd.Series([0.5, 0.9], index=[0, 4])))
        self.assertIsInstance(fa, pd.DataFrame)
        self.assertTrue(
            fa.equals(
                pd.DataFrame([[0.0, 0.5], [0.4, 0.9]], index=[5, 9], columns=["x", "y"])
            )
        )

    def test_array_series_dataframe_mismatched(self):
        n = np.array([0.0, np.nan, 0.2, 0.3, 0.4])
        s = pd.Series([0.5, 0.6, 0.7, np.nan], index=[0, 1, 2, 3])
        f = pd.DataFrame(
            [[0.0, 0.5], [0.1, 0.6], [np.nan, 0.7], [0.3, 0.8], [0.4, 0.9]],
            index=[5, 6, 7, 8, 9],
            columns=["x", "y"],
        ...
        self.assertRaisesRegex(
            ValueError, "requires all arrays to have the same", removes_nan, n, s, f
        )
