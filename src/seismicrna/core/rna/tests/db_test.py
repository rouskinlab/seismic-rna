...
from seismicrna.core.rna.db import PAIRED_MARKS, format_db_string, parse_db_string


class TestPairedMarks(ut.TestCase):
    def test_paired_marks(self):
        self.assertEqual(
            list(PAIRED_MARKS.items()), list(map(tuple, [")(", "><", "][", "}{"]))
        )
...
class TestParseDbString(ut.TestCase):
        ...
        for length in range(5):
            ...
            for seq5 in range(1, 5):
                with self.subTest(length=length, seq5=seq5):
                    self.assertEqual(parse_db_string(struct, seq5), expect)
    ...
    def test_deep_pairs(self):
        for cmark, omark in PAIRED_MARKS.items():
            for length in range(5):
                ...
                for seq5 in range(1, 5):
                    with self.subTest(
                        cmark=cmark, omark=omark, length=length, seq5=seq5
                    ):
                        expect = [
                            (seq5 + i, 2 * length + seq5 - (i + 1))
            ...
            for length in range(5):
                ...
                for seq5 in range(1, 5):
                    with self.subTest(
                        ...
                        expect = [
                            (seq5 + 2 * i, seq5 + 2 * i + 1) for i in range(length)
                        ]
                        self.assertEqual(parse_db_string(struct, seq5), expect)
    ...
    def test_dangling_opener(self):
        ...
        self.assertRaisesRegex(
            ValueError, "Position 2 has an unmatched '{'", parse_db_string, struct
        )

    def test_dangling_closer(self):
        struct = ".[.].>."
        self.assertRaisesRegex(
            ValueError, "Position 13 has an unmatched '>'", parse_db_string, struct, 8
        )
...
class TestFormatDbString(ut.TestCase):
        ...
        for length in range(11, 20):
            for seq5 in range(1, 5):
                for position in range(seq5, seq5 + 5):
                    for gap in range(3):
                        with self.subTest(
                            length=length, seq5=seq5, position=position, gap=gap
                        ):
                            ...
                            self.assertEqual(
                                format_db_string(pairs, length, seq5), expect
                            )
    ...
    def test_deep_pairs(self):
        for length in range(5):
            ...
            for seq5 in range(1, 5):
                pairs = [(seq5 + i, 2 * length + seq5 - (i + 1)) for i in range(length)]
                with self.subTest(length=length, seq5=seq5):
                    self.assertEqual(format_db_string(pairs, length * 2, seq5), expect)
    ...
    def test_shallow_pairs(self):
        for length in range(5):
            ...
            for seq5 in range(1, 5):
                with self.subTest(length=length, seq5=seq5):
                    pairs = [(seq5 + 2 * i, seq5 + 2 * i + 1) for i in range(length)]
                    self.assertEqual(format_db_string(pairs, length * 2, seq5), expect)
    ...
    def test_excess_pseudoknot(self):
        ...
        self.assertRaisesRegex(
            ValueError, "Cannot write base-pair", format_db_string, pairs, 10
        )
    ...
    def test_invalid_pair(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            "5' partner must be less than 3' partner, but got 5 ≥ 3",
            format_db_string,
            pairs,
            10,
        )
    ...
    def test_repeat_pos5(self):
        pairs = [(2, 8), (2, 5), (4, 7)]
        self.assertRaisesRegex(
            ValueError, "Got >1 base pair for position 2", format_db_string, pairs, 8
        )

    def test_repeat_pos3(self):
        pairs = [(2, 8), (3, 4), (4, 7)]
        self.assertRaisesRegex(
            ValueError, "Got >1 base pair for position 4", format_db_string, pairs, 8
        )
