...
class Rna2dStem(Rna2dPart):
    ...
    def __init__(self, side1: RNARegion, side2: RNARegion, **kwargs):
        ...
        if side1.region.length != side2.region.length:
            raise ValueError(
                f"The lengths of side 1 ({side1.region.length}) "
...
class RNAStructure(RNARegion):
    ...
    def init_args(self):
        return super().init_args | dict(
            title=self.title, pairs=self.pairs, branch=self.branch
        )
    ...
    def is_paired_internally(self):
        ...
        distance = np.abs(
            table_is_internal - table_is_internal.index.get_level_values(POS_NAME)
        )
        ...
        is_internal_pos = is_internal[is_internal].index.get_level_values(POS_NAME)
    ...
    def ct_data(self):
        ...
        return pd.DataFrame.from_dict(
            {field: pd.Series(values, index=index) for field, values in data.items()}
        )
    ...
    def db_string(self):
        ...
        return format_db_string(self.pairs, self.region.length, self.region.end5)
...
def calc_wfmi(
    ...
    require_equal("struct1.region", region, struct2.region, "struct2.region")
    ...
    return float(f_unpaired + (1.0 - f_unpaired) * fmi)
