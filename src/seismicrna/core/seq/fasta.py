        ...
        while line:
            ...
            if only is None or name in only:
                ...
                if seq_type is not None:
                    ...
                    logger.detail(f"Parsed {repr(name)} ({len(seq)} nt {item_type})")
                    yield name, seq
                else:
...
def write_fasta(
    ...
    if need_write(fasta, force):
        ...
        with NamedTemporaryFile(
            "w", dir=fasta.parent, prefix=fasta.stem, suffix=fasta.suffix, delete=False
        ) as f:
        ...
        try:
            ...
            logger.action(f"Released temporary FASTA file {tmp_fasta} to {fasta}")
        finally:
