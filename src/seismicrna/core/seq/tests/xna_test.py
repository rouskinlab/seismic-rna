...
from seismicrna.core.seq.xna import (
    InvalidBaseError,
    XNA,
    DNA,
    RNA,
...
class TestDNA(ut.TestCase):
    ...
    def test_get_comptrans(self):
        self.assertEqual(
            DNA.get_comptrans(), {65: "T", 67: "G", 71: "C", 78: "N", 84: "A"}
        )
        ...
        for char in printable:
            if char not in "ACGTNacgtn":
                self.assertRaisesRegex(
                    InvalidBaseError, "Invalid DNA bases:", DNA, char
                )
    ...
    def test_from_any_seq_valid(self):
        for length in range(1, 5):
            for bases in product(*(["ACGTUNacgtun"] * length)):
                ...
                self.assertEqual(len(dna), length)
                self.assertEqual(str(dna), "".join(bases).upper().replace("U", "T"))

    def test_from_any_seq_invalid(self):
        for char in printable:
            if char not in "ACGTUNacgtun":
                self.assertRaisesRegex(
                    InvalidBaseError, "Invalid DNA bases:", DNA.from_any_seq, char
                )
    ...
    def test_kmers(self):
        ...
        with self.subTest(n=0, k=-1):
            k = -1
            self.assertRaisesRegex(
                ValueError, f"k must be ≥ 0, but got {k}", lambda: list(seq.kmers(k))
            )
...
class TestRNA(ut.TestCase):
    ...
    def test_get_comptrans(self):
        self.assertEqual(
            RNA.get_comptrans(), {65: "U", 67: "G", 71: "C", 78: "N", 85: "A"}
        )
    ...
    def test_invalid_bases(self):
        for char in printable:
            if char not in "ACGUNacgun":
                self.assertRaisesRegex(
                    InvalidBaseError, "Invalid RNA bases:", RNA, char
                )
    ...
    def test_from_any_seq_valid(self):
        for length in range(1, 5):
            for bases in product(*(["ACGTUNacgtun"] * length)):
                ...
                self.assertEqual(len(rna), length)
                self.assertEqual(str(rna), "".join(bases).upper().replace("T", "U"))

    def test_from_any_seq_invalid(self):
        for char in printable:
            if char not in "ACGTUNacgtun":
                self.assertRaisesRegex(
                    InvalidBaseError, "Invalid RNA bases:", RNA.from_any_seq, char
                )
...
class TestXNA(ut.TestCase):
    ...
    def test_abstract_base_class(self):
        ...
        self.assertRaisesRegex(
            TypeError, "Can't instantiate abstract class XNA", XNA, "ACGN"
        )
...
class TestExpandDegenerateSeq(ut.TestCase):
    ...
    def test_zero_degenerate(self):
        ...
        self.assertEqual(
            list(expand_degenerate_seq(DNA("ACGT"))), list(map(DNA, ["ACGT"]))
        )

    def test_one_degenerate(self):
        """Test that one sequence is returned for each DNA base."""
        self.assertEqual(
            list(expand_degenerate_seq(DNA("ACNT"))),
    ...
    def test_two_degenerate(self):
        ...
        two DNA bases."""
        self.assertEqual(
            list(expand_degenerate_seq(DNA("NCGN"))),
            list(
                map(
                    DNA,
                    [
