...
from seismicrna.core.seq.fasta import (
    FASTA_NAME_MARK,
    FASTA_NAME_CHARS,
...
class TestValidFastaSeqname(ut.TestCase):
    ...
    def test_blank(self):
        prefixes = [FASTA_NAME_MARK] + [f"{FASTA_NAME_MARK}{w}" for w in whitespace]
...
class TestParseFasta(ut.TestCase):
    def test_valid_names(self):
        with NTFile("w", suffix=path.FASTA_EXTS[0], delete=False) as f:
            ...
            f.write(
                linesep.join([">Seq1", "GTACGTGNTCATC", ">Seq2 ", "AGCTGTGNNT", "ATCG"])
            )
        try:
    ...
    def test_valid_dna(self):
        ...
        try:
            ...
            remove(filepath)
        self.assertEqual(
            records, {"Seq1": DNA("GTACGTGNTCATC"), "Seq2": DNA("AGCTGTGNNTATCG")}
        )
    ...
    def test_valid_rna(self):
        with NTFile("w", suffix=path.FASTA_EXTS[0], delete=False) as f:
            filepath = Path(f.file.name)
            f.write(
                linesep.join([">Seq1", "GUACGUGNUCAUC", ">Seq2 ", "AGCUGUGNNU", "AUCG"])
            )
        try:
            ...
            remove(filepath)
        self.assertEqual(
            records, {"Seq1": RNA("GUACGUGNUCAUC"), "Seq2": RNA("AGCUGUGNNUAUCG")}
        )
    ...
    def test_duplicate_name(self):
        with NTFile("w", suffix=path.FASTA_EXTS[0], delete=False) as f:
            filepath = Path(f.file.name)
            f.write(
                linesep.join([">Seq1", "GTACGTGNTCATC", ">Seq1 ", "AGCTGTGNNT", "ATCG"])
            )
        try:
            self.assertRaisesRegex(
                DuplicateReferenceNameError,
...
class TestWriteFasta(ut.TestCase):
    def test_valid_names(self):
        seqs = [("Seq1", DNA("GTACGTGNTCATC")), ("Seq2", DNA("AGCTGTGNNTATCG"))]
        text = ">Seq1\nGTACGTGNTCATC\n>Seq2\nAGCTGTGNNTATCG\n"
