...
from seismicrna.core.validate import (
    require_issubclass,
    require_isinstance,
    require_isin,
    require_equal,
...
class TestRequireIsSubclass(ut.TestCase):
    def test_is_subclass(self):
        classes = OSError
        self.assertIsNone(require_issubclass("xyz", FileExistsError, classes))

    def test_is_subclasses(self):
        classes = OSError, int
        self.assertIsNone(require_issubclass("xyz", FileExistsError, classes))
        self.assertIsNone(require_issubclass("xyz", int, classes))

    def test_not_issubclass(self):
        classes = OSError
        self.assertRaisesRegex(
            ValueError,
            (f"xyz must be a subclass of {classes}, but got {ZeroDivisionError}"),
            require_issubclass,
            "xyz",
            ZeroDivisionError,
            classes,
    ...
    def test_not_issubclasses(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            (f"xyz must be a subclass of {classes}, but got {float}")
            .replace("(", "[(]")
            .replace(")", "[)]"),
            require_issubclass,
            "xyz",
    ...
    def test_custom_not_valueerror(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            (f"error_type must be a subclass of {ValueError}, but got {MyCustomError}"),
            require_issubclass,
            "xyz",
            ZeroDivisionError,
            classes,
...
class TestRequireIsInstance(ut.TestCase):
    def test_is_instance(self):
        classes = int
        self.assertIsNone(require_isinstance("xyz", 12, classes))

    def test_is_instances(self):
        classes = int, float
        self.assertIsNone(require_isinstance("xyz", 12, classes))
        self.assertIsNone(require_isinstance("xyz", 12.3, classes))

    def test_not_isinstance(self):
        classes = int
        self.assertRaisesRegex(
            TypeError,
            (f"xyz must be an instance of {classes}, but got 12.3 of type {float}"),
            require_isinstance,
            "xyz",
            12.3,
            classes,
    ...
    def test_not_isinstances(self):
        ...
        self.assertRaisesRegex(
            TypeError,
            (f"xyz must be an instance of {classes}, but got '12.3' of type {str}")
            .replace("(", "[(]")
            .replace(")", "[)]"),
            require_isinstance,
            "xyz",
    ...
    def test_custom_not_valueerror(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            (f"error_type must be a subclass of {TypeError}, but got {MyCustomError}"),
            require_isinstance,
            "xyz",
            12.3,
            classes,
...
class TestRequireIsIn(ut.TestCase):
    ...
    def test_not_isin_named(self):
        self.assertRaisesRegex(
            ValueError,
            ("xyz must be in letters, but got xyz='d' and letters='abc'"),
            require_isin,
            "xyz",
            "d",
            "abc",
...
class TestRequireEqual(ut.TestCase):
    ...
    def test_equal_wrong_type_value(self):
        self.assertRaisesRegex(
            TypeError,
            (f"a must be an instance of {int}, but got '0' of type {str}"),
            require_equal,
            "a",
            "0",
            1,
    ...
    def test_equal_wrong_type_other_unnamed(self):
        self.assertRaisesRegex(
            TypeError,
            (f"other must be an instance of {int}, but got '1' of type {str}"),
            require_equal,
            "a",
            0,
            "1",
    ...
    def test_equal_wrong_type_other_named(self):
        self.assertRaisesRegex(
            TypeError,
            (f"b must be an instance of {int}, but got '1' of type {str}"),
            require_equal,
            "a",
            0,
            "1",
    ...
    def test_not_equal_int_unnamed(self):
        self.assertRaisesRegex(
            ValueError, "Must have a = 1, but got 0", require_equal, "a", 0, 1
        )

    def test_not_equal_str_unnamed(self):
        self.assertRaisesRegex(
            ValueError, "Must have a = 'A', but got 'B'", require_equal, "a", "B", "A"
        )
...
class TestRequireAtLeast(ut.TestCase):
    ...
    def test_not_atmost(self):
        self.assertRaisesRegex(
            ValueError, "Must have a ≥ 1, but got 0", require_atleast, "a", 0, 1
        )

    def test_atleast_wrong_type_minimum_unnamed(self):
        self.assertRaisesRegex(
            TypeError,
            (f"minimum must be an instance of {int}, but got '0' of type {str}"),
            require_atleast,
            "a",
            1,
            "0",
...
class TestRequireAtMost(ut.TestCase):
    ...
    def test_not_atmost(self):
        self.assertRaisesRegex(
            ValueError, "Must have a ≤ 0, but got 1", require_atmost, "a", 1, 0
        )

    def test_atmost_wrong_type_maximum_unnamed(self):
        self.assertRaisesRegex(
            TypeError,
            (f"maximum must be an instance of {int}, but got '1' of type {str}"),
            require_atmost,
            "a",
            0,
            "1",
