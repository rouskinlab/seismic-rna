...
class TestExcInfo(ut.TestCase):
    ...
    def test_exc_info(self):
        for verbosity in [Level.STATUS, Level.WARNING, Level.ERROR, Level.FATAL]:
            set_config(verbosity=verbosity)
            self.assertFalse(exc_info())
        for verbosity in [Level.DETAIL, Level.ROUTINE, Level.TASK]:
            set_config(verbosity=verbosity)
    ...
    def test_log_file(self):
        ...
        try:
            ...
            with open(log_file_path) as log_file:
                ...
                self.assertTrue(lines[0].startswith(f"LOGMSG> {Level.ROUTINE.name}"))
                self.assertEqual(lines[1], f"{msg1}\n")
                self.assertEqual(lines[2], "\n")
                self.assertTrue(lines[3].startswith(f"LOGMSG> {Level.DETAIL.name}"))
