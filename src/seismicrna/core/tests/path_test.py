...
class TestGetSeismicRNASourceDir(ut.TestCase):
    def test_get_seismicrna_source_dir(self):
        ...
        self.assertEqual(seismicrna_source_dir, sanitize(__file__).parent.parent.parent)
...
class TestValidateBranches(ut.TestCase):
    def test_validate_branches(self):
        ...
        self.assertIsNone(validate_branches({"step_1": "a1", "step_2": "b1"}))
        self.assertIsNone(validate_branches({"step_1": "a1", "step_2": "a1"}))
        ...
        self.assertRaisesRegex(
            PathValueError, "txt cannot be empty string", validate_branches, {"": "a1"}
        )
        self.assertRaisesRegex(
            PathValueError,
...
class TestAddBranch(ut.TestCase):
    def test_add_branches(self):
        self.assertDictEqual(add_branch("step1", "", {}), {"step1": ""})
        self.assertDictEqual(
            add_branch("step2", "", {"step1": "a1"}), {"step1": "a1", "step2": ""}
        )
        self.assertDictEqual(add_branch("step1", "b1", {}), {"step1": "b1"})
        self.assertDictEqual(
            add_branch("step2", "b1", {"step1": "a1"}), {"step1": "a1", "step2": "b1"}
        )
        self.assertDictEqual(
            add_branch("step2", "a1", {"step1": "a1"}), {"step1": "a1", "step2": "a1"}
        )
        self.assertDictEqual(
            add_branch("step3", "b1", {"step1": "a1", "step2": "a2"}),
            {"step1": "a1", "step2": "a2", "step3": "b1"},
        )
        self.assertRaisesRegex(
            PathValueError, "txt cannot be empty string", add_branch, "", "a1", {}
        )
        self.assertRaisesRegex(
            PathValueError,
            r"branch 'a_1' has illegal characters: \['_'\]",
...
class TestBranchesField(ut.TestCase):
    ...
    def test_branches_build(self):
        ...
        self.assertEqual(BranchesField.build(["branch1"]), "_branch1")
        self.assertEqual(BranchesField.build(["branch1"]), "_branch1")
        self.assertEqual(
            BranchesField.build(["branch1", "branch2"]), "_branch1_branch2"
        )
        self.assertRaisesRegex(
            PathTypeError,
            (
    ...
    def test_branches_parse(self):
        ...
        self.assertListEqual(BranchesField.parse("branch-1"), ["branch-1"])
        self.assertListEqual(BranchesField.parse("_branch-1"), ["branch-1"])
        self.assertListEqual(BranchesField.parse("branch-1_"), ["branch-1"])
        self.assertListEqual(BranchesField.parse("_branch-1_"), ["branch-1"])
        self.assertListEqual(
            BranchesField.parse("_branch1_branch2"), ["branch1", "branch2"]
        )
        self.assertListEqual(
            BranchesField.parse("_branch1__branch2"), ["branch1", "branch2"]
        )
        self.assertListEqual(
            BranchesField.parse("_branch1__branch2_"), ["branch1", "branch2"]
        )
        self.assertListEqual(
            BranchesField.parse("_branch1___branch2_"), ["branch1", "branch2"]
        )
        self.assertRaisesRegex(
            PathTypeError,
            (
...
class TestCmdSeg(ut.TestCase):
    def test_cmdseg_build_branches_list(self):
        self.assertEqual(StepSeg.build({STEP: "align", BRANCHES: []}), "align")
        self.assertEqual(
            StepSeg.build({STEP: "align", BRANCHES: ["branch1"]}), "align_branch1"
        )
        self.assertEqual(
            StepSeg.build({STEP: "align", BRANCHES: ["branch1", "branch2"]}),
        ...
        self.assertRaisesRegex(
            ...
            (
                ...
                r"'filterscan', 'clusterscan', 'fold', 'duplex', 'graph'\], but got 'malign'"
            ),
            StepSeg.build,
    ...
    def test_cmdseg_build_branches_dict(self):
        self.assertEqual(StepSeg.build({STEP: "align", BRANCHES: {}}), "align")
        self.assertEqual(
            StepSeg.build({STEP: "align", BRANCHES: {"step1": ""}}), "align"
        )
        self.assertEqual(
            StepSeg.build({STEP: "align", BRANCHES: {"step1": "branch1"}}),
            "align_branch1",
        )
        self.assertEqual(
            StepSeg.build({STEP: "align", BRANCHES: {"step1": "", "step2": "branch1"}}),
            "align_branch1",
        )
        self.assertEqual(
            StepSeg.build(
                {STEP: "align", BRANCHES: {"step1": "branch1", "step2": "branch2"}}
            ),
            "align_branch1_branch2",
        )
        self.assertRaisesRegex(
    ...
    def test_cmdseg_parse(self):
        self.assertEqual(StepSeg.parse("align"), {STEP: "align", BRANCHES: []})
        self.assertEqual(
            StepSeg.parse("align_branch1"), {STEP: "align", BRANCHES: ["branch1"]}
        )
        self.assertEqual(
            StepSeg.parse("align__branch1"), {STEP: "align", BRANCHES: ["branch1"]}
        )
        self.assertEqual(
            StepSeg.parse("align_branch1_"), {STEP: "align", BRANCHES: ["branch1"]}
        )
        self.assertEqual(
            StepSeg.parse("align_branch1_branch2"),
            {STEP: "align", BRANCHES: ["branch1", "branch2"]},
        )
        self.assertEqual(
            StepSeg.parse("align__branch1_branch2"),
            {STEP: "align", BRANCHES: ["branch1", "branch2"]},
        )
        self.assertEqual(
            StepSeg.parse("align_branch1__branch2"),
            {STEP: "align", BRANCHES: ["branch1", "branch2"]},
        )
        self.assertRaisesRegex(
            PathValueError, "Could not parse fields in text ''", StepSeg.parse, ""
        )
        self.assertRaisesRegex(
            PathValueError,
        ...
        self.assertRaisesRegex(
            ...
            (
                r"option must be in \['demult', 'align', 'idmut', 'filter', 'cluster', "
                r"'filterscan', 'clusterscan', 'fold', 'duplex', 'graph'\], but got 'alight'"
            ),
            StepSeg.parse,
            "alight",
        )
...
class TestSymlinkIfNeeded(ut.TestCase):
    ...
    def test_target_not_exist(self):
        ...
        for link_exists in range(2):
            self.assertRaisesRegex(
                FileNotFoundError, str(target), symlink_if_needed, link, target
            )
    ...
    def test_link_not_symlink(self):
        ...
        target.mkdir()
        self.assertRaisesRegex(
            OSError, f"{link} is not a symbolic link", symlink_if_needed, link, target
        )
