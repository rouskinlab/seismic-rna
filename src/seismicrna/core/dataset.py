...
class Dataset(ABC):
    ...
    def link_data_dirs_to_tmp(self, tmp_dir: Path):
        ...
        for data_dir in self.data_dirs:
            tmp_data_dir = path.transpath(tmp_dir, self.top, data_dir, strict=True)
...
class UnbiasDataset(Dataset, ABC):
    ...
    def __init__(
        self, *args, masked_read_nums: dict[int, list] | None = None, **kwargs
    ):
...
class LoadFunction(object):
    ...
    def __init__(self, dataset_type: type[Dataset], /, *more_types: type[Dataset]):
    ...
    def _get_dataset_type_consensus(self, method: Callable[[type[Dataset]], Any]):
    ...
    def build_report_path(self, path_fields: dict[str, Any]):
        ...
        return path.build(
            self.report_path_seg_types, {**self.report_path_auto_fields, **path_fields}
        )
    ...
    def __call__(self, report_file: str | Path, **kwargs):
        ...
        errmsg = "\n".join(
            f"{type_name}: {error}" for type_name, error in errors.items()
        )
    ...
    def iterate(
        ...
        **kwargs,
        ...
        for report_file in path.find_files_chain(
            input_path, self.report_path_seg_types
        ):
            try:
    ...
    def __str__(self):
        names = ", ".join(dataset_type.__name__ for dataset_type in self.dataset_types)
...
class LoadedDataset(Dataset, ABC):
    ...
    def get_batch_path(self, batch_num: int):
        ...
        return path.cast_path(
            ...
            {path.BATCH: batch_num, path.EXT: path.BRICKLE_EXT},
        )
...
class TallDataset(MergedDataset, ABC):
    ...
    def datasets(self):
        ...
        return list(map(self.get_dataset_load_func(), sample_report_files))
...
class WideDataset(MergedRegionDataset, ABC):
    ...
    def datasets(self):
        ...
        if not region_report_files:
            raise NoDataError(f"{self} has no datasets")
        return list(map(self.get_dataset_load_func(), region_report_files))
    ...
    def region(self):
        return unite(
            self._list_dataset_attr("region"),
...
class MultistepDataset(MutsDataset, ABC):
    ...
    @classmethod
    def get_dataset1_report_file(cls, dataset2_report_file: Path, verify_times: bool):
    ...
    def load_dataset1(cls, dataset2_report_file: Path, verify_times: bool):
        ...
        load_func = cls.get_dataset1_load_func()
        return load_func(
            cls.get_dataset1_report_file(dataset2_report_file, verify_times),
    ...
    def load_dataset2(cls, dataset2_report_file: Path, verify_times: bool):
        ...
        return load_func(dataset2_report_file, verify_times=verify_times)
