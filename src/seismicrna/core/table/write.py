...
class Tabulator(ABC):
    ...
    def get_load_function(cls):
        ...
        load_functions = list(
            set(table_type.get_load_function() for table_type in cls.table_types())
        )
    ...
    def _format_table(cls, counts: pd.DataFrame, header: Header):
        ...
        table.loc[:, INFOR_REL] = (
            table.loc[:, MATCH_REL].values + table.loc[:, MUTAT_REL].values
        )
        return table
    ...
    def data_per_pos(self):
        ...
        return self._format_table(self.counts_per_pos, self.pos_header).reindex(
            self.region.range
        )
    ...
    def data_per_read(self):
        ...
        return self._format_table(self.counts_per_read, self.read_header)
    ...
    def generate_tables(
        self, *, pos: bool = True, read: bool = True, clust: bool = True
    ):
...
class CountTabulator(Tabulator, ABC):
    ...
    def __init__(self, *, batch_counts: Iterable[tuple[Any, Any, Any, Any]], **kwargs):
...
class DatasetTabulator(BatchTabulator, ABC):
    ...
    def _list_args(cls, func: Callable):
        ...
        return [
            ...
            if (param.kind is Parameter.KEYWORD_ONLY and param.default is param.empty)
        ]
    ...
    def init_kws(cls):
        ...
        super().__init__()."""
        return ["top", "sample", "branches", "get_batch_count_all", "num_batches"]

    def __init__(self, *, dataset: MutsDataset, validate: bool = False, **kwargs):
        ...
        super().__init__(
            **{attr: getattr(dataset, attr) for attr in self.init_kws()},
            validate=validate,
            **kwargs,
        )
