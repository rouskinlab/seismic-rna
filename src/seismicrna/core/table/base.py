...
def all_patterns(subpattern: RelPattern | None = None):
    ...
    return {
        rel: get_subpattern(rel, subpattern)
...
def get_rel_name(rel_code: str):
    ...
    try:
        ...
        raise ValueError(
            f"Relationship code must be one of {list(REL_CODES)}, "
...
def _get_denom_cols(numer_cols: pd.Index):
    ...
    if numer_cols.name is not None and numer_cols.name != REL_NAME:
        raise ValueError(
            f"Expected index to be named {repr(REL_NAME)}, "
            f"but got {repr(numer_cols.name)}"
        )
    return pd.Index(list(map(_get_denom_rel, numer_cols)), name=numer_cols.name)
...
class Table(path.HasRefFilePath, ABC):
    ...
    def path(self):
        ...
        field_values = dict()
        for field in path.get_fields_in_seg_types(
            self.get_path_seg_types(), include_top=True
        ):
            try:
    ...
    def header(self):
        ...
        if not isinstance(header, self.get_header_type()):
            raise TypeError(
                f"Expected {self.get_header_type().__name__}, "
...
class RelTypeTable(Table, ABC):
    ...
    @classmethod
    def _format_data(cls, data: pd.DataFrame, *, precision: int | None, squeeze: bool):
        ...
        if squeeze:
            ...
            if data.columns.size != 1:
                raise ValueError(
                    "Only data with exactly 1 column can be "
    ...
    @abstractmethod
    def _fetch_data(
        self, columns: pd.Index, exclude_masked: bool = False
    ) -> pd.DataFrame:
...
class PositionTable(RelTypeTable, ABC):
    ...
    def _fetch_data(self, columns: pd.Index, exclude_masked: bool = False):
        return (
            self.data.loc[self.region.unmasked, columns]
            if exclude_masked
    ...
    def _resample_clust(
        ...
        *,
        ...
        def resample_multi(
            ...
            if n_any_rel.ndim != 1:
                raise ValueError(
                    f"n_any_rel must have exactly 1 dimension, but got {n_any_rel.ndim}"
                )
            if p_any_rel.ndim != 1:
                raise ValueError(
                    f"p_any_rel must have exactly 1 dimension, but got {p_any_rel.ndim}"
                )
            if tolerance < 0.0:
            ...
            n_each_rel = pd.DataFrame(
                n_every_rel[:, : len(rel_kinds)],
        ...
        p_sub = self.fetch_ratio(rel=SUBST_REL, squeeze=True, **fetch_kwargs).values
    ...
    def resample(
        ...
        *,
        ...
        for k, clust in self.header.clusts:
            ...
            resampled_clust = self._resample_clust(
                k, clust, n_cov, seed=rng.integers(max_seed)
            )
