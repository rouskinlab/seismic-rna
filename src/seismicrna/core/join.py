    ...
    if pos_muts := muts.get(position):
        if add_pos_muts := add_muts.get(position):
            ...
            reads, counts = np.unique(
                np.concatenate(list(joined_pos_muts.values())), return_counts=True
            )
            if counts.max(initial=0) > 1:
                raise ValueError(
                    f"Reads {reads[counts > 1]} have > 1 type of "
...
def _join_attrs(attrs: dict[str, Any], add_attrs: dict[str, Any], region: Region):
    ...
    read_indexes, add_read_indexes = locate_elements(
        union_read_nums,
        attrs[READ_NUMS],
        add_attrs[READ_NUMS],
        what="read_nums",
    ...
    _, num_segs = match_reads_segments(attrs[SEG_END5S], attrs[SEG_END3S], None)
    _, add_num_segs = match_reads_segments(
        add_attrs[SEG_END5S], add_attrs[SEG_END3S], None
    )
    ...
    attrs[MUTS] = {
        pos: _join_position(muts, add_muts, pos) for pos in region.unmasked_int
    }
...
class JoinMutsDataset(WideMutsDataset, ABC):
    ...
    def check_batch_type(cls, batch: MutsBatch):
        ...
        if not isinstance(batch, cls.get_batch_type()):
            raise TypeError(
                f"Expected {cls.get_batch_type().__name__}, "
