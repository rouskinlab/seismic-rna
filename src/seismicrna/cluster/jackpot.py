...
def _sim_clusters_dropped(p_clust_given_read: np.ndarray, seed: int | None):
    ...
    np.ndarray
        ...
        read.
    """
    # Import numpy and jackpot_jit here so that importing this module does not
    # import numpy and numba (which is slow).
    import numpy as np
    from .jackpot_jit import assign_clusters_dropped
    ...
    assign_clusters_dropped(clusters, p_clust_given_read, n_reads_per_clust, rng)
    return clusters
...
def _sim_muts_dropped(
    ...
    # import numpy and numba (which is slow).
    import numpy as np
    from .jackpot_jit import sim_muts_dropped_jit
...
def _sim_muts_merged(
    ...
    # import numpy and numba (which is slow).
    import numpy as np
    from .jackpot_jit import sim_muts_merged_jit
    ...
    # rates are a contiguous 1D row inside the JIT inner loop.
    p_mut_t = np.ascontiguousarray(p_mut.T)
    sim_muts_merged_jit(muts, end5s, end3s, clusts, p_mut_t, min_mut_gap, rng)
    return muts
...
def _sim_reads_dropped(
    ...
    seeds = get_random_integer_generator(seed)
    clusts = _sim_clusters_dropped(
        p_clust_given_ends_noclose[end5s, end3s], next(seeds)
    )
...
def bootstrap_jackpot_scores(
    ...
    logger.debug(
        "Began bootstrapping null jackpotting scores for a dataset "
        "with {} reads and a real jackpotting score of {}",
        n_reads,
        real_jackpot_score,
    )
    ...
    for _ in range(max_jackpot_sims):
        ...
        logger.trace("Null jackpotting score: {}", null_jackpotting_score)
        ...
        if not np.isnan(jq_ci_lo) and not np.isnan(jq_ci_up):
            logger.trace("{}: {} - {}", conf_pct, jq_ci_lo, jq_ci_up)
        ...
        logger.warning(
            "After the maximum of {} simulations, the "
            "{} is {} - {}, which still contains "
            "the maximum jackpotting quotient {}, "
    ...
    logger.debug("Ended bootstrapping null jackpotting scores")
