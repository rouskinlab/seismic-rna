...
class EMRunsK(object):
    ...
    def __init__(
        ...
        self.run_not_overclustered = np.array(
            [
                not (
                    (
                        run.max_pearson > max_pearson_run
        ...
        self.run_not_underclustered = np.array(
            [not (run.jackpot_quotient > max_jackpot_quotient) for run in runs]
        )
        # Select the best run.
        ...
        self.converged = np.array(
            [run.iter if run.converged else NOCONV for run in runs]
        )
        ...
        self.jackpot_quotients = np.array([run.jackpot_quotient for run in runs])
        # Maximum ARCD between the clusters and average.
        self.max_arcds_vs_ens_avg = np.array([run.max_arcd_vs_ens_avg for run in runs])
        ...
        self.marcds_vs_best = np.array(
            [
                calc_mean_arcsine_distance_clusters(
                    run.mus.values, self.best.mus.values
                )
                for run in runs
            ]
        )
        ...
        self.pearsons_vs_best = np.array(
            [
                calc_mean_pearson_clusters(run.mus.values, self.best.mus.values)
                for run in runs
            ]
        )
    ...
    def __str__(self):
        return (
            f"{type(self).__name__} with {self.k} clusters and {self.n_runs_total} runs"
        )
...
def assign_clusterings(mus1: np.ndarray, mus2: np.ndarray):
    ...
    if n1 >= 1 and k1 >= 1:
        ...
        costs = np.array(
            [
                [
                    calc_sum_arcsine_distance(mus1[:, cluster1], mus2[:, cluster2])
                    for cluster2 in range(k2)
                ]
                for cluster1 in range(k1)
            ]
        ).reshape((k1, k2))
...
def calc_mean_arcsine_distance_clusters(mus1: np.ndarray, mus2: np.ndarray):
    ...
    return float(calc_mean_arcsine_distance(*_assign_concat_clusters(mus1, mus2)))
