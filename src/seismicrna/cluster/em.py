...
def _calc_bic(
    n_params: int, n_data: int, log_like: float, min_data_param_ratio: float = 2.0
):
    ...
    if n_data == 0:
        logger.warning(
            "The Bayesian Information Criterion (BIC) is undefined for 0 data points"
        )
...
class EMRun(object):
    ...
    def __init__(
        ...
        *,
        ...
        if not max_iter >= min_iter:
            raise ValueError(
                f"max_iter must be ≥ min_iter ({min_iter}), but got {max_iter}"
            )
        self._max_iter = max_iter
        # Cutoff for convergence of EM
        if not em_thresh >= 0.0:
    ...
    def _n_params(self):
        ...
        df_ends = max(
            0, np.count_nonzero(self._p_ends[np.triu_indices_from(self._p_ends)]) - 1
        )
    ...
    def _exp_step(self):
        ...
        log_like = _calc_log_like(self._logp_marginal, self._counts_per_uniq)
