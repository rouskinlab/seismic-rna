...
class TestMarginalResps(ut.TestCase):
    def compare(
        ...
        result_log_marginals, result_resps = calc_marginal_resps(
            p_mut,
            p_ends,
            p_clust,
            end5s,
    ...
    def test_2pos_gap0(self):
        ...
        expect_log_marginals = np.log([0.18, 0.02, 0.36, 0.04, 0.09, 0.01, 0.24, 0.06])
    ...
    def test_2pos_masked0(self):
        ...
        expect_log_marginals = np.log([0.20, 0.20, 0.40, 0.40, 0.10, 0.10, 0.24, 0.06])
        expect_resps = np.ones((8, 1))
    ...
    def test_2pos_masked1(self):
        ...
        expect_log_marginals = np.log([0.18, 0.02, 0.45, 0.05, 0.45, 0.05, 0.30, 0.30])
        expect_resps = np.ones((8, 1))
        for min_mut_gap in [0, 1]:
            for mut_collisions in ["drop", "merge"]:
                self.compare(
    ...
    def test_1pos_2clusters(self):
        ...
        expect_resps = np.array([[3.0 / 7.0, 4.0 / 7.0], [1.0 / 4.0, 3.0 / 4.0]])
        for mut_collisions in ["drop", "merge"]:
            self.compare(
                p_mut,
                p_ends,
    ...
    def test_2pos_gap0_2clusters(self):
        p_mut = np.array([[0.1, 0.4], [0.2, 0.3]])
        p_ends = np.array([[0.2, 0.5], [0.0, 0.3]])
        p_clust = np.array([0.4, 0.6])
        end5s = np.array([0, 0, 0, 0, 0, 0, 1, 1])
        end3s = np.array([0, 0, 1, 1, 1, 1, 1, 1])
    ...
    def test_2pos_gap1_merge(self):
        ...
        unmasked = np.array([0, 1])
        muts_per_pos = [np.array([1, 3]), np.array([4, 6])]
        expect_log_marginals = np.log([0.18, 0.02, 0.36, 0.04, 0.10, 0.24, 0.06])
        expect_resps = np.ones((7, 1))
        min_mut_gap = 1
        self.compare(
            p_mut,
    ...
    def test_6pos_gap2_2clusters_drop(self):
        (p_mut, p_ends, p_clust, end5s, end3s, unmasked, muts_per_pos, min_mut_gap) = (
...
class TestCalcMarginalSimulated(ut.TestCase):
    ...
    @staticmethod
    def _empirical_counts(muts: np.ndarray, end5s: np.ndarray, end3s: np.ndarray):
        """Return (unique_end5s, unique_end3s, unique_muts, counts)
        for the unique (end5, end3, mutation pattern) tuples."""
        keys = [
            (int(a), int(b), tuple(row.tolist()))
    ...
    def _check(
        ...
        self.assertTrue(check.any(), "no bins have enough counts to test")
