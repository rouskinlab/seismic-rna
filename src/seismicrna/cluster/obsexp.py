...
def assemble_log_obs_exp(uniq_reads: UniqReads, ks: list[EMRunsK]):
    ...
    log_exp = [
        (
            format_exp_count_col(runs.k),
            pd.Series(runs.best.log_exp, uniq_reads.uniq_names),
        )
        for runs in ks
    ]
    # Assemble all log counts into one DataFrame.
    log_obs_exp = pd.DataFrame.from_dict(
        {LOG_OBS_NAME: uniq_reads.log_obs} | dict(log_exp)
    )
        ...
        try:
            ...
            if not np.isnan(jackpot_quotient) and not np.isnan(max_log_obs):
                ...
                fig.add_annotation(
                    ...
                    y=max_log_obs,
                    text=" = ".join(
                        map(str, [JACKPOT_QUOTIENT, round(jackpot_quotient, PRECISION)])
                    ),
                    xanchor="left",
                    yanchor="bottom",
                    showarrow=False,
