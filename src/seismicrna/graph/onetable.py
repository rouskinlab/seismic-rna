...
class OneTableGraph(TableGraph, OneSourceGraph, ABC):
    ...
    def __init__(self, *, table: Table | PositionTable | AbundanceTable, **kwargs):
        """
        Parameters
        ----------
...
class OneTableRelClusterGroupGraph(
    OneTableGraph, RelTableGraph, OneSourceClusterGroupGraph, ABC
):
    @cached_property
    def row_tracks(self):
        return make_tracks(
...
class OneTableRunner(TableRunner, ABC):
    ...
    def run(
        ...
        **kwargs
        ...
        writers = [
            writer_type(table_file)
            for table_file in cls.load_input_files(
...
class OneTableRelClusterGroupRunner(
