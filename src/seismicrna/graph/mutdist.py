...
def _init_hists(dataset: MutsDataset, rel_name: str):
    ...
    return pd.DataFrame(
        ...
        pd.RangeIndex(_get_num_bins(dataset), name=HIST_COUNT_NAME),
        header.index,
    )
    ...
    *,
    ...
    if dataset.is_clustered:
        ...
        for (k, clust), weights in batch.read_weights.items():
            ...
            hists.loc[:, col] += np.bincount(
                min_mut_dist, weights=weights, minlength=num_bins
            )
    else:
        if batch.read_weights is not None:
            raise TypeError(batch.read_weights)
        hists.loc[:, rel_name] += np.bincount(min_mut_dist, minlength=num_bins)
    if count_pos_ends:
        # Also count the relationships per position and 5'/3' ends.
        batch_counts = batch.count_all(
            patterns=all_patterns(dataset.pattern),
            ks=dataset.ks,
            count_ends=True,
            count_pos=True,
            count_read=False,
...
class MutationDistanceGraph(DatasetGraph, ColorMapGraph):
    ...
    @cached_property
    def table(self):
        tables = list(self.tabulator.generate_tables(pos=True, read=False, clust=False))
        assert len(tables) == 1
        table = tables[0]
    ...
    def loc_clusters(self):
        return (
            self.k if self.k is not None else slice(None),
    ...
    def g_test(self):
        ...
        if self.calc_null:
            ...
            if dof >= 1:
                ...
                p_value = 1.0 - chi2.cdf(g_stat, dof)
            else:
            ...
            p_value = np.nan
        return (
            pd.Series(g_stat, index=self._real_hist.columns),
    ...
    def get_traces(self):
        if self.calc_null:
            for row, ((_, real_values), (_, null_values)) in enumerate(
                zip(self._real_hist.items(), self._null_hist.items(), strict=True),
                start=1,
            ):
                yield (row, 1), get_hist_trace(real_values, self.rel_name, self.cmap)
                yield (
                    (row, 1),
                    get_hist_trace(
                        null_values, get_null_name(self.rel_name), self.cmap
                    ),
                )
        else:
            for row, (_, real_values) in enumerate(self._real_hist.items(), start=1):
                yield (row, 1), get_hist_trace(real_values, self.rel_name, self.cmap)


class MutationDistanceWriter(DatasetWriter):
    def get_graph(self, rel, **kwargs):
        return MutationDistanceGraph(dataset=self.dataset, rel=rel, **kwargs)
