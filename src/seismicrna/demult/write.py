...
ALLOWED_SUFFIXES = sorted(
    (ext for ext in FQ_ALL_EXTS if ext.startswith(".")), key=len, reverse=True
)


def check_demult_fqs(
    demult_fqs: dict[tuple[str, str], FastqUnit],
    out_dir: Path,
    ...
    for (sample, ref), fq_unit in demult_fqs.items():
        ...
        if fq_unit.paired:
            fqs_found = 0
            for exts, segs in zip(
                (path.FQ1_EXTS, path.FQ2_EXTS), (path.DMFASTQ1_SEGS, path.DMFASTQ2_SEGS)
            ):
                for ext in exts:
                    demult_expect = path.build(
                        segs,
                        {
                            path.TOP: out_dir,
    ...
    **kwargs,
    ...
    if refs_meta:
        barcodes = RefBarcodes(
            ref_seqs=parse_fasta(fasta, DNA),
    ...
    if force:
        ...
        fqs_to_demult, demult_fqs_extant = list_demulted_fqs(
            demult_fqs, out_dir, branches
        )
    ...
    if fqs_to_demult:
        ...
        logger.warning(
            "All given FASTQ files have already been demultiplexed: "
...
def demult_fq_pipeline(
    ...
    **kwargs,
    ...
    demult_parts = dispatch(
        ...
        raise_on_error=False,
        args=args,
        kwargs=dict(release_dir=release_dir, **kwargs),
    )
    ...
    arg_parts = [
        arg_part for arg_part in assembled_parts if any(fq.exists() for fq in arg_part)
    ]
    dispatch(
        merge_parts,
        num_cpus=num_cpus,
    ...
    out_path = release_to_out(
        out_dir=out_dir,
        release_dir=release_dir,
        initial_path=release_dir / Path(fq_inp.sample) / "demult",
    )

    return out_path


def split_fq(fq_inp: FastqUnit, working_dir: Path, num_split: int):
    ...
    for fq in fq_inp.paths.values():
        ...
        if num_split == 1:
            split_path = (split_dir / strip_all_fq_suffixes(fq.name)).with_suffix(
                ".part_001" + "".join(fq.suffixes)
            )
...
def get_split_paths(split_dir, fq_inp, num_parts):
    ...
    return [tuple(rename_fq_part(fq) for fq in fq_tuple) for fq_tuple in split_paths]
...
def process_fq_part(
    ...
    **kwargs,
    ...
    fq_unit_part = FastqUnit(
        **fq_unit_args, phred_enc=fq_inp.phred_enc, one_ref=fq_inp.one_ref
    )
    ...
    for fq_type, fq_path in fq_unit_part.paths.items():
        ...
        seg_pattern = next(
            (
                pattern
                for pattern in fq_seg_patterns
                if path.path_matches(fq_path.name, [pattern])
            ),
            None,
        )
        if not seg_pattern:
    ...
    out_fqs = {
        name: FastqUnit(**paths, phred_enc=fq_inp.phred_enc, one_ref=fq_inp.one_ref)
...
def demult_ahocorasick(
    ...
    count = 0
    total = 0
    for segs, recs in fq_unit.iter_records(
        segments=[barcodes.read_pos_range, barcodes.rc_read_pos_range]
    ):
        seqs = [" ".join(seg) for seg in segs]
        bcs = [
            (match, match_rc)
            for match, match_rc in zip_longest(
                automaton.iter(seqs[0]), rc_automaton.iter(seqs[1])
            )
        ]
...
def merge_parts(parts=list[Path]):
    ...
    if not all(
        ("_" in part.name and part.name.split("_", 1)[1] == base_name) for part in parts
    ):
