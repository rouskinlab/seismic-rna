    ...
    for i, meta in enumerate(lines, start=1):
        ...
        try:
            ...
            if not pd.isnull(ref):
                ...
                raise ValueError(
                    f"Either {FIELD_REF} or {FIELD_NAME} column must be filled"
                )
            ...
            if has_coords and has_bc:
                raise ValueError(
                    f"Got both coordinates ({end5}, {end3}) "
...
class RefBarcodes(object):
        ...
        *,
        ...
        if refs_meta_file is not None:
            try:
                ...
                logger.error(error)
        all_names = [
            key for d in (meta_coords, meta_bcs, cli_coords, cli_bcs) for key in d
        ]
        ...
        self._bcs = {
            ref: (bc.pop() if isinstance(bc, set) else bc)
            for ref, bc in self._bcs.items()
        }
    ...
    def read_pos_range(self):
        ...
        max_pos = (
            max(self.read_positions) + self.max_barcode_len + self.index_tolerance
        )  # TODO: Max barcode len is not with max read_position
    ...
    def slice_position(self):
        ...
        for read_position, barcode in zip(self.read_positions, self.barcodes):
            ...
            )  # +1 for the space character inserted between reads.
    ...
    def rc_read_positions(self):
        ...
        return [
            self.ref_lengths.get(name, 0) - (read_pos + len(bc))
            for name, (bc, read_pos) in self._bcs.items()
        ]  # TODO: Fix rc indexing of non-ref barcodes
    ...
    def rc_read_pos_range(self):
        ...
        max_pos = (
            max(self.rc_read_positions) + self.max_barcode_len + self.index_tolerance
        )
        return (min_pos, max_pos)
    ...
    def get_automaton(
    ...
    ):  # TODO: May be able to remove the check argument depending on edge cases.
        ...
        for barcode_idx, (name, barcode) in enumerate(barcodes):
            ...
            if self.mismatches:
                barcode_set = get_neighbors(
                    barcode, max_mismatches=self.mismatches, allow_n=self.allow_n
                )
            else:
    ...
    def valid_positions(self):
        ...
        if self.index_tolerance > 0:
            valid_positions = {
                ref_idx: expand_by_tolerance(read_pos, self.index_tolerance)
                for ref_idx, read_pos in zip(np.arange(num_refs), slice_positions)
            }
        else:
            valid_positions = {
                ref_idx: read_pos
    ...
    @cached_property
    def automaton(self):
        return self.get_automaton(
            [(name, barcode) for name, barcode in zip(self.names, self.barcodes)],
            start=self.automaton_index,
        )
    ...
    def rc_automaton(self):
        return self.get_automaton(
            [
                (name, rc_barcode)
                for name, rc_barcode in zip(self.names, self.rc_barcodes)
            ],
            start=self.automaton_index,
        )
