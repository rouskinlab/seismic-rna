...
from seismicrna.relate.sim import simulate_batches, make_p_ends_2d, calc_pmut_pattern
from seismicrna.sim.relate import parse_min_mut_gap_weights
...
class TestParseMinMutGapWeights(ut.TestCase):
    ...
    def test_multiple_pairs(self):
        self.assertEqual(parse_min_mut_gap_weights("0:0.3,2:0.7"),
                         {0: 0.3, 2: 0.7})
    ...
    def test_multiple_zero_weights_excluded(self):
        self.assertEqual(parse_min_mut_gap_weights("0:0.5,2:0.0,4:0.5"),
                         {0: 0.5, 4: 0.5})
...
class TestSimulateBatches(ut.TestCase):
    ...
    def _make_pmut(self, seed: int):
        ...
        pmut = pd.DataFrame(0., index=row_index, columns=col_index)
        pmut[MATCH] = p_match
        pmut[self.AMB] = p_amb
        pmut[SUB_T] = p_mut
        return pmut
    ...
    def _run(self, mut_collisions: str, seed: int,
             min_mut_gap: int = 0, min_mut_gap_weights: dict | None = None):
        return list(simulate_batches(
            ...
            min_mut_gap_weights=(self.P_GAP if min_mut_gap_weights is None
                                 else min_mut_gap_weights),
    ...
    def _expected_drop_with_weights(self, seed: int):
        ...
        p_noclose_given_gap_clust = np.stack([
            calc_p_noclose_given_clust(
                p_ends_2d,
                calc_p_noclose_given_ends_auto(pmut_given_clust, gap)
            )
        ...
        return np.array([
            ...
            for p_clust_val, p_gap_val
            in product(p_clust_given_noclose, p_gap_given_noclose)
        ])
            ...
            with self.subTest(mut_collisions=mut_collisions):
                ...
                for act, exp in zip(actual, expected, strict=True):
                    self.assertAlmostEqual(
                        act / exp, 1.0, delta=self.TOLERANCE,
    ...
    def _expected_drop_no_weights(self, seed: int, min_mut_gap: int):
        ...
        p_noclose_given_clust = calc_p_noclose_given_clust(
            p_ends_2d,
            calc_p_noclose_given_ends_auto(pmut_given_clust, min_mut_gap)
        )
        pclust = np.array(self.PCLUST)
        p_clust_given_noclose = calc_p_clust_given_noclose(
            pclust, p_noclose_given_clust
    ...
    def test_drop_read_counts_without_gap_weights(self):
        ...
        rng = np.random.default_rng(seed=0)
        seed = int(rng.integers(2 ** 32))
        batches = self._run(MUT_COLLISIONS_DROP, seed,
                            min_mut_gap=min_mut_gap, min_mut_gap_weights={})
        ...
        self.assertAlmostEqual(
            actual.sum() / self.NUM_READS, 1.0, delta=self.TOLERANCE,
