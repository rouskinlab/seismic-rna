...
def verify_proportions(p: Any):
    ...
    if (min_p := arr.min()) < 0.0:
        raise ValueError(f"Every proportion must be ≥ 0, but minimum is {min_p}")
    if (max_p := arr.max()) > 1.0:
        raise ValueError(f"Every proportion must be ≤ 1, but maximum is {max_p}")
...
def make_pmut_means(
    *,
    ...
    pas = [pac, pag, pat]
    verify_proportions(pas + [(pad := 1.0 - sum(pas))])
    a = pd.Series(
        {
            SUB_C: pam * phiq * pac,
            SUB_G: pam * phiq * pag,
            SUB_T: pam * phiq * pat,
            DELET: pam * pad,
            ANY_B: (1.0 - pam * pad) * ploq,
        }
    )
    # Mutations at C bases.
    pcs = [pca, pcg, pct]
    verify_proportions(pcs + [(pcd := 1.0 - sum(pcs))])
    c = pd.Series(
        {
            SUB_A: pcm * phiq * pca,
            SUB_G: pcm * phiq * pcg,
            SUB_T: pcm * phiq * pct,
            DELET: pcm * pcd,
            ANY_D: (1.0 - pcm * pcd) * ploq,
        }
    )
    # Mutations at G bases.
    pgs = [pga, pgc, pgt]
    verify_proportions(pgs + [(pgd := 1.0 - sum(pgs))])
    g = pd.Series(
        {
            SUB_A: pgm * phiq * pga,
            SUB_C: pgm * phiq * pgc,
            SUB_T: pgm * phiq * pgt,
            DELET: pgm * pgd,
            ANY_H: (1.0 - pgm * pgd) * ploq,
        }
    )
    # Mutations at T bases.
    pts = [pta, ptc, ptg]
    verify_proportions(pts + [(ptd := 1.0 - sum(pts))])
    t = pd.Series(
        {
            SUB_A: ptm * phiq * pta,
            SUB_C: ptm * phiq * ptc,
            SUB_G: ptm * phiq * ptg,
            DELET: ptm * ptd,
            ANY_V: (1.0 - ptm * ptd) * ploq,
        }
    )
    # Mutations at N bases.
    n = pd.Series({DELET: pnm, ANY_N: (1.0 - pnm) * ploq})
    pmut_means = pd.DataFrame.from_dict(
        {BASEA: a, BASEC: c, BASEG: g, BASET: t, BASEN: n}
    ).fillna(0.0)
    pmut_means.loc[MATCH] = 1.0 - pmut_means.sum(axis=0)
    return pmut_means
...
def sim_pmut(
    ...
    if not isinstance(positions, pd.MultiIndex):
        raise TypeError(
            f"positions must be a MultiIndex, but got {type(positions).__name__}"
        )
    if not isinstance(mean, pd.DataFrame):
        raise TypeError(f"mean must be a DataFrame, but got {type(mean).__name__}")
    ...
    pmut = pd.DataFrame(0.0, index=positions, columns=rels)
    for base in DNA.alph():
        ...
        mean_nonzero = mean.loc[mean[base] != 0.0, base]
        rel_var = relative_variance[base.lower()]
        var_nonzero = rel_var * (mean_nonzero * (1.0 - mean_nonzero))
        # Simulate the mutation rates.
        num_nonzero = np.count_nonzero(mean_nonzero)
        if num_nonzero > 1:
            pmut_base = rng.dirichlet(
                calc_dirichlet_params(mean_nonzero.values, var_nonzero.values),
                size=base_pos.size,
            )
        elif num_nonzero == 1:
            pmut_base = rng.beta(
                *calc_beta_params(mean_nonzero.values[0], var_nonzero.values[0]),
                size=base_pos.size,
            )
        else:
            pmut_base = np.broadcast_to(
                mean_nonzero.values[np.newaxis, :], (base_pos.size, mean_nonzero.size)
            )
...
def run_struct(
    ...
    if need_write(pmut_file, force):
        ...
        index = get_shared_index(structure.table.index for structure in structures)
        ...
        if ref_coords or ref_primers:
            regs = [
                RegionFinder(ref, seq, seq5=seq5, end5=e5, end3=e3)
                for e5, e3 in ref_coords
            ] + [
                RegionFinder(
                    ref, seq, seq5=seq5, fwd=fwd, rev=rev, exclude_primers=True
                )
        ...
        def update_mus(pair_: tuple[int, int]):
            ...
            mu_paired[pair_] = sim_pmut(
                use_index, pm, vp, reg_end5, reg_end3, seed=next(seeds)
            )
            mu_unpaired[pair_] = sim_pmut(
                use_index, um, vu, reg_end5, reg_end3, seed=next(seeds)
            )
        ...
        header = make_header(rels=map(str, rels), ks=[num_structures])
        pmut = pd.DataFrame(np.nan, index, header.index)
        for structure, cluster in zip(
            structures, list_clusts(num_structures), strict=True
        ):
            ...
            for rel in rels:
                ...
                for position, paired in structure.is_paired.items():
                    ...
                    pmut.at[position, (str(rel), num_structures, cluster)] = mu[
                        tuple(enclosing.loc[position])
                    ].at[position, rel]
