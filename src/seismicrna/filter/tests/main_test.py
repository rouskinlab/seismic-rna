...
def write_datasets(out_dir: Path,
    ...
    for sample, num_reads in reads_per_sample.items():
        ...
        for batch, i_to_batch in enumerate(i_to_batches):
            ...
            checksums[IDmutBatchIO.btype()].append(
                idmut_batch.save(out_dir)[1]
            )
...
class TestFilter(ut.TestCase, ABC):
    ...
    def dataset(self, *,
                ...
                **kwargs):
        filter_dir, = run_filter([self._report_file],
                             count_del=count_del,
                             count_ins=count_ins,
...
class TestFilterSingle1Sample1Batch(TestFilterSingle,
                                  TestFilter1Sample,
                                  TestFilter1Batch):
