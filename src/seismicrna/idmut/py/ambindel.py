...
class Indel(ABC):
    ...
    def __str__(self):
        return (
            f"{type(self).__name__} at {self.opposite} {self.lateral5, self.lateral3}"
        )
...
class Deletion(Indel):
    def _calc_rels(
        ...
        rel_del = encode_rel(ref_base_del, read_base, read_qual, min_qual)
        rel_opp = encode_rel(ref_base_opp, read_base, read_qual, min_qual)
    ...
    def try_move(
        ...
        """
        (swap_lateral, next_lateral3, next_opposite) = self._calc_positions(move5to3)
        if _check_out_of_bounds(next_opposite, ref_end5, ref_end3):
            return False
        if _check_collisions(pods, self.pod, next_lateral3, move5to3):
            return False
        rel_del, rel_opp = self._calc_rels(
            ref_seq, read_seq, read_qual, min_qual, swap_lateral, next_opposite
        )
...
class Insertion(Indel):
    def _calc_rels(
        ...
        # base in the read.
        rel_ins = encode_rel(ref_base, read_base_ins, read_qual_ins, min_qual)
        rel_opp = encode_rel(ref_base, read_base_opp, read_qual_opp, min_qual)
    ...
    def try_move(
        ...
        if _check_collisions(pods, self.pod, next_lateral3, move5to3):
            return False
        rel_ins, rel_opp = self._calc_rels(
            ref_seq, read_seq, read_qual, min_qual, swap_lateral, next_opposite
        )
        # Determine if the relationships before and after moving the
        ...
        if move5to3 == insert3:
            ...
            if rel_ins != MATCH or all(
                ins.get_lateral(insert3) != init_lateral for ins in self.pod.indels
            ):
                ...
                rels[self.get_lateral(not insert3)] = (
                    rels.get(self.get_lateral(not insert3), MATCH) | rel_ins
                )
            rels[self.get_lateral(insert3)] = rels.get(
                self.get_lateral(insert3), MATCH
            ) | get_ins_rel(insert3)
        else:
            # The insertion moves to the opposite side as it is marked.
            (swap_lateral_, next_lateral3_, next_opposite_) = self._calc_positions(
                insert3
            )
            rel_ins_, rel_opp_ = self._calc_rels(
                ref_seq, read_seq, read_qual, min_qual, swap_lateral_, next_opposite_
            )
            # Move the indels.
            _move_indels(self, next_opposite, next_lateral3)
            # Check after the move.
            if rel_opp_ != MATCH or all(
                ins.get_lateral(insert3) != init_lateral for ins in self.pod.indels
            ):
                # If there is another insertion next to the base that just
                # took the place of the insertion that moved, then do not
...
def _check_collisions(
    pods: list[IndelPod], pod: IndelPod, next_lateral3: int, move5to3: bool
):
    ...
    if 0 <= next_pod_index < len(pods):
        ...
        return (
            next_indel.opposite == next_lateral3
...
def _move_indels(indel: Indel, opposite: int, lateral3: int):
    ...
    for other in indel.pod.indels:
        if (
            indel.opposite < other.opposite < opposite
