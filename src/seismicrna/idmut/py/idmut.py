...
class SamFlag(object):
    ...
    def __init__(self, flag: int):
        ...
        if not 0 <= flag <= MAX_FLAG:
            raise IDmutError("SAM flag is too large")
        self.flag = flag
...
class SamRead(object):
    ...
    def __init__(self, line: str):
        ...
        try:
            ...
            raise IDmutError("Failed to parse read name") from None
        # Bitwise flag
        try:
            self.flag = SamFlag(int(fields[1]))
        except (IndexError, ValueError):
            raise IDmutError("Failed to parse SAM flag") from None
        # Reference name
        try:
            self.ref = fields[2]
            if not self.ref:
            ...
            raise IDmutError("Failed to parse reference name") from None
        # Mapping position
        try:
            self.pos = int(fields[3])
        except (IndexError, ValueError):
            raise IDmutError("Failed to parse mapping position") from None
        if self.pos < 1:
            raise IDmutError("Mapping position is 0")
        # Mapping quality
        try:
            self.mapq = int(fields[4])
        except (IndexError, ValueError):
            raise IDmutError("Failed to parse mapping quality") from None
        # CIGAR string
        try:
            self.cigar = fields[5]
            if not self.cigar:
            ...
            raise IDmutError("Failed to parse CIGAR string") from None
        # Read sequence
        try:
            self.seq = fields[9]
            if not self.seq:
            ...
            raise IDmutError("Failed to parse read sequence") from None
        # Read quality
        try:
...
def _calc_rels_read(read: SamRead,
    ...
    ----------
    read: SamRead
        Read from SAM file to be processed
    ref_seq: str
    ...
    if read.pos > ref_length:
        raise IDmutError("Mapping position exceeds length of reference")
    ...
    if ref_bases == 0:
        raise IDmutError("CIGAR operations consumed 0 bases in the reference")
        ...
        if (cigar_op == CIG_ALIGN
            ...
            for _ in range(op_length):
                if ref_pos == init_ref_pos:
                    raise IDmutError("A deletion was the first relationship")
                ref_pos += 1  # 1-indexed now until this iteration ends
                if ref_pos - init_ref_pos >= ref_bases:
                    raise IDmutError("A deletion was the last relationship")
                rels[ref_pos] = DELET
            ...
            ref_pos += op_length
            if ref_pos - init_ref_pos >= ref_bases:
                raise IDmutError("An intron was the last relationship")
            ...
            assert read_pos > 0
            if ref_pos - init_ref_pos >= ref_bases:
                raise IDmutError("An insertion was the last relationship")
    ...
    if clip_end5 < 0:
        raise IDmutError(f"clip_end5 must be ≥ 0, but got {clip_end5}")
    if clip_end3 < 0:
        raise IDmutError(f"clip_end3 must be ≥ 0, but got {clip_end3}")
...
def _validate_read(read: SamRead,
    ...
    """
    if read.ref != ref:
        raise IDmutError("Reference name does not match name of SAM file")
    if read.mapq < min_mapq:
        raise IDmutError("Mapping quality is insufficient")
...
def _validate_pair(read1: SamRead, read2: SamRead):
    ...
    if not read1.flag.read1 or read1.flag.read2:
        raise IDmutError("Mate 1 is not marked as READ1")
    if not read2.flag.read2 or read2.flag.read1:
        raise IDmutError("Mate 2 is not marked as READ2")
...
def _merge_rels(end5sf: list[int],
    ...
    for pos in relsf | relsr:
        ...
        if rel != MATCH:
            if rel == NOCOV:
                raise IDmutError(f"Cannot merge non-covered position {pos}")
            merged_rels[pos] = rel
    return merged_rels
