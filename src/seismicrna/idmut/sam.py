...
def _iter_batch_indexes(sam_file: Path, batch_size: int, paired: bool):
    ...
    with open(sam_file) as f:
        ...
        while line := f.readline():
            ...
            logger.detail(f"Batch {batch} of {sam_file}: {batch_start} - {position}")
...
def _iter_records_single(sam_file: Path, start: int, stop: int):
    ...
    with open(sam_file) as f:
        ...
        while f.tell() < stop:
            ...
            if paired:
                raise ValueError(f"Read {repr(name)} in {sam_file} is not single-end")
            yield name, line, ""
...
def _iter_records_paired(sam_file: Path, start: int, stop: int):
    ...
    with open(sam_file) as f:
        ...
        while f.tell() < stop:
            ...
            if not paired:
                raise ValueError(f"Read {repr(name)} in {sam_file} is not paired-end")
            if prev_line:
...
class XamViewer(object):
    ...
    def _iter_batch_indexes(self):
        ...
        self.create_tmp_sam()
        yield from _iter_batch_indexes(self.tmp_sam_path, self.batch_size, self.paired)
