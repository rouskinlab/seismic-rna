...
def _iter_batch_indexes(sam_file: Path, batch_size: int, paired: bool):
    ...
    """
    with logger.debug.begin("calculating batch indexes in {}", sam_file):
        if batch_size <= 0:
            raise ValueError(
        ...
        with open(sam_file) as f:
            ...
            while line := f.readline():
                ...
                logger.trace("Batch {} of {}: {} - {}", batch, sam_file, batch_start, position)
...
def _iter_records_single(sam_file: Path, start: int, stop: int):
    ...
    with logger.debug.begin("iterating through single-end records in {}", sam_file):
        with open(sam_file) as f:
            f.seek(start)
            while f.tell() < stop:
                line = f.readline()
...
def _iter_records_paired(sam_file: Path, start: int, stop: int):
    ...
    with logger.debug.begin("iterating through paired-end records in {}", sam_file):
        with open(sam_file) as f:
            f.seek(start)
            prev_line = ""
            prev_name = ""
...
class XamViewer(object):
    ...
    def _iter_batch_indexes(self):
        ...
        with logger.debug.begin("computing batch indexes for {}", self):
            self.create_tmp_sam()
    ...
    def iter_records(self, batch: int):
        ...
        with logger.debug.begin("iterating records for {} batch {}", self, batch):
