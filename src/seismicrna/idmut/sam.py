...
def _iter_batch_indexes(sam_file: Path, batch_size: int, paired: bool):
    ...
    """
    with logger.debug.single_context("calculating batch indexes in {}", sam_file):
        if batch_size <= 0:
            raise ValueError(
        ...
        with open(sam_file) as f:
            ...
            while line := f.readline():
                ...
                if paired and line:
                    ...
                    if line := f.readline():
                        ...
                        if not (read_paired and next_paired):
                            raise ValueError(
                                f"{sam_file} does not have only paired-end reads"
                            )
                ...
                logger.trace(
                    "Batch {} of {}: {} - {}", batch, sam_file, batch_start, position
                )
...
def _iter_records_single(sam_file: Path, start: int, stop: int):
    ...
    with logger.debug.single_context(
        "iterating through single-end records in {}", sam_file
    ):
        with open(sam_file) as f:
            f.seek(start)
            while f.tell() < stop:
...
def _iter_records_paired(sam_file: Path, start: int, stop: int):
    ...
    with logger.debug.single_context(
        "iterating through paired-end records in {}", sam_file
    ):
        with open(sam_file) as f:
            f.seek(start)
            prev_line = ""
            ...
            while f.tell() < stop:
                ...
                if not paired:
                    raise ValueError(
                        f"Read {repr(name)} in {sam_file} is not paired-end"
                    )
                if prev_line:
            ...
            if prev_line:
                ...
                if prev_proper:
                    raise ValueError(
                        f"Read {repr(prev_name)} in {sam_file} is "
...
def tmp_xam_cmd(xam_in: Path, xam_out: Path, paired: bool, num_cpus: int = 1):
    ...
    if paired:
        ...
        # Remove the header.
        view_step = view_xam_cmd(
            None, xam_out, flags_req=flags_req, flags_exc=flags_exc
        )
        return cmds_to_pipe([collate_step, view_step])
    # Remove the header.
    flags_exc |= FLAG_PAIRED
    return view_xam_cmd(xam_in, xam_out, flags_req=flags_req, flags_exc=flags_exc)


run_tmp_xam = ShellCommand(
    "collating reads and creating temporary SAM file", tmp_xam_cmd
)


class SamFileViewer(object):
    def __init__(
        self,
        xam_input: Path,
        tmp_dir: Path,
        ...
        """
        Initialize a SamFileViewer for iterating over batches of reads.
    ...
    def ancestors(self):
        ...
        if len(ancestors_flat) > 1:
            raise ValueError(
                f"{self} must have ≤ 1 ancestor branches, but got {ancestors_flat}"
            )
    ...
    @cached_property
    def branches(self):
        return path.add_branch(path.IDMUT_STEP, self.branch, self.ancestors)
    ...
    def _iter_batch_indexes(self):
        ...
        with logger.debug.single_context("computing batch indexes for {}", self):
            self.create_tmp_sam()
            yield from _iter_batch_indexes(
                self.tmp_sam_path, self.batch_size, self.paired
            )

    @cached_property
    def indexes(self):
        return {
            batch: (start, stop) for batch, start, stop in self._iter_batch_indexes()
        }

    def iter_records(self, batch: int):
        """Iterate through the records of the batch."""
        with logger.debug.single_context(
            "iterating records for {} batch {}", self, batch
        ):
