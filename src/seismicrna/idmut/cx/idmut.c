...
/* Encode a relationship (rel) as a match or substitution as such if the
quality is sufficient, otherwise as ambiguous. */
static inline unsigned char encode_rel(char ref_base,
                                       char read_base,
                                       char read_qual,
...
static inline int get_next_cigar_op(CigarOp *cigar)
{
    ...
    if (cigar->len == 0)
    {
        ...
        if (*cigar->op)
        {
            PyErr_SetString(IDmutError, "Invalid CIGAR operation");
            return -1;
        }
...
typedef struct
{
    ...
    EndsArray end5s;      // 5' ends of segments
    EndsArray end3s;      // 3' ends of segments
} SamRead;
...
static int parse_ulong(unsigned long *ulong, const char *str)
{
    ...
    if (str == NULL)
    {
        ...
        PyErr_SetString(IDmutError, "(Got NULL string to parse as ulong)");
        return -1;
    }
    ...
    *ulong = strtoul(str, &endptr, DECIMAL);
    ...
    if (endptr == str)
    {
        ...
        PyErr_SetString(IDmutError, "(Parsed zero characters as ulong)");
        return -1;
    }
                          ...
                          size_t ref_len)
{
    ...
    {
        PyErr_SetString(IDmutError, "Failed to parse read name");
        return -1;
    }
    ...
    {
        PyErr_SetString(IDmutError, "Failed to parse SAM flag");
        return -1;
    }
    read->flag = temp_ulong;
    if (read->flag > MAX_FLAG)
    {
        PyErr_SetString(IDmutError, "SAM flag is too large");
        return -1;
    }
    ...
    {
        PyErr_SetString(IDmutError, "Failed to parse reference name");
        return -1;
    }
    ...
    if (parse_ulong(&temp_ulong, next_sam_field(&end)))
    {
        PyErr_SetString(IDmutError, "Failed to parse mapping position");
        return -1;
    }
    read->pos = (size_t)temp_ulong;
    if (read->pos == 0)
    {
        PyErr_SetString(IDmutError, "Mapping position is 0");
        return -1;
    }
    if (read->pos > ref_len)
    {
    ...
    if (parse_ulong(&temp_ulong, next_sam_field(&end)))
    {
        PyErr_SetString(IDmutError, "Failed to parse mapping quality");
        return -1;
    }
    ...
    {
        PyErr_SetString(IDmutError, "Failed to parse CIGAR string");
        return -1;
    }
    ...
    {
        PyErr_SetString(IDmutError, "Failed to parse next reference name");
        return -1;
    }
    ...
    if (next_sam_field(&end) == NULL)
    {
        PyErr_SetString(IDmutError, "Failed to parse next mapping position");
        return -1;
    }
    ...
    if (next_sam_field(&end) == NULL)
    {
        PyErr_SetString(IDmutError, "Failed to parse template length");
        return -1;
    }
    ...
    {
        PyErr_SetString(IDmutError, "Failed to parse read sequence");
        return -1;
    }
                         ...
                         const SamRead *read2)
{
    ...
    {
        PyErr_SetString(IDmutError, "Mates 1 and 2 have different names");
        return -1;
    }
    ...
    {
        PyErr_SetString(IDmutError, "Mate 1 is not marked as READ1");
        return -1;
    }
    if (!(read2->read2) || read2->read1)
    {
        PyErr_SetString(IDmutError, "Mate 2 is not marked as READ2");
        return -1;
    }
...
/* Identify the mutations in one line of a SAM file.
Do NOT call this function on a SamRead that has not yet been initialized
    ...
    while (cigar.op != NULL)
    {
        ...
        switch (op)
        {
            ...
            case CIG_SUBST:
                ...
                {
                    read->rels[rels_pos] = encode_rel(rels_seq[rels_pos],
                                                      read->seq[read_pos],
                                                      read->quals[read_pos],
                                                      min_qual);
                    rels_pos++;
                         ...
                         PyObject *ends_rels_tuple)
{
    ...
    // Return NULL so that py_id_muts_lines can "return cleanup(...);"
    return NULL;
}


/* Python interface to function id_muts_lines */
static PyObject *py_id_muts_lines(PyObject *self, PyObject *args)
{
...
/* Python method table */
static PyMethodDef IDmutMethods[] = {
    {"id_muts_lines",
     py_id_muts_lines,
     METH_VARARGS,
...
static struct PyModuleDef idmutmodule = {
    PyModuleDef_HEAD_INIT,
    "idmut",      // module name
    NULL,         // module documentation
    -1,           // module state
    IDmutMethods  // method table
};


/* Python module initialization function */
PyMODINIT_FUNC PyInit_idmut(void)
{
    // Initialize the module and ensure it exists.
    PyObject *module;
    module = PyModule_Create(&idmutmodule);
    if (module == NULL)
        {return NULL;}
