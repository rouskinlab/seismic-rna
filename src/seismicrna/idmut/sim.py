...
def _calc_p_noclose_given_clust(
    pmut_given_clust: np.ndarray, p_ends: np.ndarray, min_mut_gap: int
) -> float:
...
def simulate_batch(
    ...
    **kwargs,
    ...
    idmut_batch = IDmutBatchIO.simulate(
        sample=sample, branches=branches, ref=ref, batch=batch, **kwargs
    )
    if write_read_names:
        name_batch = ReadNamesBatchIO.simulate(
            sample=sample,
            branches=branches,
            ref=ref,
            batch=batch,
            num_reads=idmut_batch.num_reads,
            formatter=formatter,
        )
    else:
        name_batch = None
    return idmut_batch, name_batch


def simulate_cluster(
    first_batch: int, batch_size: int, num_reads: int, seed: int | None, **kwargs
):
    ...
    for batch in range(first_batch, last_batch):
        yield simulate_batch(
            batch=batch, num_reads=batch_size, seed=next(seeds), **kwargs
        )
    # Simulate the last batch, which may have fewer reads.
    if last_batch_size > 0:
        yield simulate_batch(
            batch=last_batch, num_reads=last_batch_size, seed=next(seeds), **kwargs
        )


def calc_pmut_pattern(pmut: pd.DataFrame, pattern: RelPattern, normalize: bool = True):
    ...
    fmut = pd.DataFrame(0.0, pmut.index, header.get_clust_header().index)
    ...
    **kwargs
    ...
    if min_mut_gap_weights:
        ...
        if mut_collisions == MUT_COLLISIONS_DROP:
            ...
            p_ends_2d = make_p_ends_2d(pends, uniq_end5s, uniq_end3s, pmut.index)
            ...
            if not np.isclose(p_gap.sum(), 1.0):
                raise ValueError(
                    f"min_mut_gap_weights must sum to 1, but got {p_gap.sum()}"
                )
            ...
            # Shape: (num_gaps, num_clusters)
            num_reads_final = (
                num_reads * p_gap[:, np.newaxis] * p_clust_given_noclose[np.newaxis, :]
            )
            ...
            flat = rng.multinomial(n_sim, (num_reads_sim / num_reads_sim.sum()).ravel())
        ...
        num_reads_clusters = pd.DataFrame(
            flat.reshape(shape), index=gap_keys, columns=pclust.index
        )
        ...
        if mut_collisions == MUT_COLLISIONS_DROP:
            ...
            num_reads_clusters = pd.Series(
                rng.multinomial(num_reads, pclust), pclust.index
            )
...
def simulate_idmut(
    ...
    **kwargs,
    ...
    branches = path.add_branch(path.IDMUT_STEP, branch, dict())
    report_file = IDmutReport.build_path(
        {path.TOP: out_dir, path.SAMPLE: sample, path.BRANCHES: branches, path.REF: ref}
    )
    if need_write(report_file, force):
        began = datetime.now()
        # Write the reference sequence to a file.
        refseq_file = RefseqIO(sample=sample, branches=branches, ref=ref, refseq=refseq)
        _, refseq_checksum = refseq_file.save(
            tmp_dir, brotli_level=brotli_level, force=True
        )
        # Simulate and write the batches.
        checksums = {IDmutBatchIO.btype(): list()}
        if write_read_names:
            checksums[ReadNamesBatchIO.btype()] = list()
        read_count = 0
        for idmut_batch, name_batch in simulate_batches(
            sample=sample,
            branches=branches,
            ref=ref,
            ...
            **kwargs,
            ...
            _, idmut_checksum = idmut_batch.save(
                tmp_dir, brotli_level=brotli_level, force=True
            )
            checksums[IDmutBatchIO.btype()].append(idmut_checksum)
            if write_read_names:
                assert isinstance(name_batch, ReadNamesBatchIO)
                _, name_checksum = name_batch.save(
                    tmp_dir, brotli_level=brotli_level, force=True
                )
                checksums[ReadNamesBatchIO.btype()].append(name_checksum)
            else:
                assert name_batch is None
            read_count += idmut_batch.num_reads
        if write_read_names:
            assert ReadNamesBatchIO.btype() in checksums
            assert len(checksums[ReadNamesBatchIO.btype()]) == len(
                checksums[IDmutBatchIO.btype()]
            )
        else:
        ...
        report = IDmutReport(
            ...
            n_batches=len(checksums[IDmutBatchIO.btype()]),
            checksums=checksums,
