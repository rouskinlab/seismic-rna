...
def iter_relvecs_q53(
    ...
    for pos in region.range_int:
        ...
        if pos in low_qual:
            ...
            opts = [
                encode_rel(ref_base, read_base, HI_QUAL, OK_QUAL)
    ...
    for rels in product(*rel_opts):
        ...
        if (max_ins is None or max_ins > 0) and not low_qual:
            no_ins5_pos = {
                ...
                if rel == DELET
                for pos in range(del_pos - 1, del_pos + 1)
            }
            ins5_pos = sorted(all5_positions - no_ins5_pos)
            for num_ins in range(
                1,
                1
                + min(max_ins if max_ins is not None else len(ins5_pos), len(ins5_pos)),
            ):
...
def iter_relvecs_all(refseq: DNA, insert3: bool = True, max_ins: int | None = None):
    """
    For a given reference sequence, yield every possible unambiguous
    ...
    for end5, end3 in cwr(range(1, len(refseq) + 1), 2):
        ...
        for n_low_qual in range((end3 - (end5 - 1)) + 1):
            ...
            for low_qual in combinations(range(end5, end3 + 1), n_low_qual):
                ...
                yield from iter_relvecs_q53(
                    refseq, low_qual, end5, end3, insert3, max_ins
                )
