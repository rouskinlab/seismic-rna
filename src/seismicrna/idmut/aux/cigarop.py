...
class CigarOp(object):
    ...
    def __init__(self, op: str):
        if op not in (CIG_ALIGN, CIG_MATCH, CIG_SUBST, CIG_DELET, CIG_INSRT, CIG_SCLIP):
...
def count_cigar_read(cigar_string: str):
    ...
    return sum(olen for op, olen in parse_cigar(cigar_string) if op_consumes_read(op))


def count_cigar_ref(cigar_string: str):
    """Count the reference positions consumed by a CIGAR string."""
    return sum(olen for op, olen in parse_cigar(cigar_string) if op_consumes_ref(op))


def count_cigar_muts(cigar_string: str):
    """Count the mutations in a CIGAR string."""
    return sum(olen for op, olen in parse_cigar(cigar_string) if op_is_mutation(op))
