...
def _calc_sample_mus(idmut_dataset, num_cpus: int):
    ...
    if len(table_files) > 1:
        raise ValueError(
            f"Expected at most 1 position table in {idmut_dataset.dir}, "
...
def pool_samples(
    ...
    *,
    ...
    pool_report_file = PoolReport.build_path(
        {
            path.TOP: out_dir,
    ...
    if need_write(pool_report_file, force):
        ...
        if pool_report_file.is_file():
            ...
            try:
                ...
                raise TypeError(
                    f"Cannot overwrite {pool_report_file} with "
        ...
        pool_dataset = load_idmut_dataset(
            write_report(tmp_dir, **report_kwargs), verify_times=verify_times
        )
        tabulate(
            pool_dataset,
        ...
        release_to_out(out_dir, tmp_dir, write_report(tmp_dir, **report_kwargs).parent)
...
def run(
    ...
    *,
    ...
    for dataset in load_idmut_dataset.iterate(input_path, verify_times=verify_times):
