...
def joined_filter_report_exists(
    top: Path,
    sample: str,
    ...
    logger.trace("Joined filter report {} already exists", filter_report_file)
    return True
...
def write_report(report_type: type[JoinReport], out_dir: Path, **kwargs):
...
def join_regions(
    ...
    *,
    ...
    if max(reg_counts.values()) > 1:
        logger.warning(
            f"Joined region {repr(joined_region)} of sample {repr(sample)}, "
            f"reference {repr(ref)} in {out_dir} got duplicate "
            f"regions: {reg_counts}"
        )
    regs = sorted(reg_counts)
    report_kwargs = dict(
        sample=sample, ref=ref, reg=joined_region, joined_regions=regs, began=began
    )
    ...
    join_report_file = report_type.build_path(
        {
            path.TOP: out_dir,
            path.SAMPLE: sample,
    ...
    if need_write(join_report_file, force):
        ...
        if join_report_file.is_file():
            ...
            try:
                ...
                raise TypeError(
                    f"Overwriting {join_report_file} with "
        ...
        dataset = load_function(
            write_report(report_type, tmp_dir, **report_kwargs),
            verify_times=verify_times,
        )
        tabulate(
            dataset,
        ...
        release_to_out(
            out_dir, tmp_dir, write_report(report_type, tmp_dir, **report_kwargs).parent
        )
...
def run(
    ...
    *,
    ...
    load_funcs = {False: load_filter_dataset, True: load_cluster_dataset}
    for clustered, load_func in load_funcs.items():
        for dataset in load_func.iterate(input_path, verify_times=verify_times):
            ...
            logger.trace("Added regions {} for {}", regs, dataset)
    ...
    for (top, sample, branches_flat, ref, clustered), regs in list(joins.items()):
        if clustered and (
            force
            or not joined_filter_report_exists(
                top, sample, branches_flat, ref, joined_region, regs
            )
        ):
    ...
    for use_clustered in [False, True]:
        args = [
            (out_dir, joined_region, sample, branches_flat, ref, regs, clustered)
            for (out_dir, sample, branches_flat, ref, clustered), regs in joins.items()
