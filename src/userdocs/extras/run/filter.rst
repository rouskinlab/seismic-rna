...
Filter: Define mutations and regions to filter reads and positions
--------------------------------------------------------------------------------

Filter: Input files
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Filter input file: IDmut report
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

You can give any number of IDmut report files as inputs for the Filter step.
See :doc:`../inputs` for ways to list multiple files.

For example, to select from relation vectors of reads from ``sample-1`` on
references ``ref-1`` and ``ref-2``, and from ``sample-2`` on reference
``ref-1``, use the command ::

    seismic filter {out}/sample-1/idmut/ref-1 {out}/sample-1/idmut/ref-2 {out}/sample-2/idmut/ref-1

where ``{out}`` is the path of your output directory from the IDmut step.

To select from all relation vectors in ``{out}``, you can use the command ::
...
Filter setting: Choose a chemical probe
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Filter setting: Define regions
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

You can use the full reference sequences or select specific regions.
...
Filter setting: Define mutations
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

The Filter step takes in relation vectors -- which encode relationships including
ambiguous mutations -- and outputs bit vectors, wherein each position in each
read has a binary, mutated/matched status.
For more information on relation vectors, see :doc:`../../data/idmut/codes`.
...
Filter setting: Exclude positions
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
(see `Filter setting: Choose a chemical probe`_):
...
Filter setting: Filter positions
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
All output files go into the directory ``{out}/{sample}/filter/{ref}/{reg}``,
where ``{out}`` is the output directory, ``{sample}`` is the sample, ``{ref}``
is the reference, and ``{reg}`` is the region.

Filter output file: Batch of masked reads
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Each batch of masked reads contains a ``FilterBatchIO`` object and is saved to the
file ``filter-batch-{num}.brickle``, where ``{num}`` is the batch number.
See :doc:`../../formats/data/brickle` for more information on brickle files.

Filter output file: Filter report
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

SEISMIC-RNA also writes a report file, ``filter-report.json``, that records the
settings you used for running the Filter step and summarizes the results, such as
which and how many positions and reads were filtered out for each reason.
See :doc:`../../formats/report/filter` for more information.

Filter: Troubleshoot and optimize
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. _filter_too_many_reads:

Too many reads are filtered out
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

In the Filter report file, check the settings for filtering reads and the number
of reads removed by each filter.

- If the settings appear too strict, then rerun the Filter step using new settings
...
- If you are losing too many reads for having too many mutations, or mutations
  ...
  - Your sequencing run gave low-quality base calls (check the FastQC reports)
    that you did not trim (in Align) or flag as ambiguous (in IDmut).
  ...
  - In the Filter step, you did not pre-exclude problematic positions, such as
    sites of endogenous RNA modifications.

Too many positions are filtered out
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

In the Filter report file, check the settings for filtering positions and the
...
- If you are losing too many positions for having too few informative reads,
  ...
  - You lost too many reads during filtering; see :ref:`filter_too_many_reads`.
...
Filter crashes or hangs while producing few or no batch files
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
- Rerun IDmut with smaller batches (with ``--batch-size``) to limit the size of
