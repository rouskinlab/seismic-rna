...
IDmut: Identify relationships between references and aligned reads
--------------------------------------------------------------------------------
...
IDmut input file: Reference sequences
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
IDmut input file: Alignment maps
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
.. note::
    ...
    Discrepancies can cause the IDmut step to fail or produce erroneous output.
    You can assume that the references match if you use the same (unmodified)
    FASTA file for both the ``align`` and ``idmut`` commands, or if you run
...
``ref-1``, use the following command::
    ...
    seismic idmut {refs.fa} sample-1/align/ref-1.cram sample-1/align/ref-2.cram sample-2/align/ref-1.cram
...
IDmut settings shared with alignment
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Because you can begin the SEISMIC-RNA workflow at ``seismic align`` or, if you
already have alignment map files, can begin at ``seismic idmut``, these two
commands share the options ``--phred-enc``, ``--min-mapq``, ``--min-reads``, and
``--out-dir``.

IDmut setting: Minimum Phred score
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

In the IDmut step, you can flag bases with low quality scores as ambiguous, as
if they were ``N``\s.
...
reads from their ends, while the minimum quality score in the IDmut step flags
low-quality bases located anywhere in the reads, while preserving read lengths.
See :ref:`idmut_low_qual` for a more detailed description of how this works.
...
IDmut setting: Ambiguous insertions and deletions
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
algorithm with ``--no-ambrel``, which will speed up the IDmut step at the cost
of reducing its accuracy on indels.

IDmut setting: Batch size
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

In the IDmut step, you can divide up your data into batches to speed up the
...
IDmut uses the batch size to calculate the number of reads in each batch.
...
(i.e. within the insert) are processed.

IDmut: Output files
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

All output files go into the directory ``{out}/{sample}/idmut/{ref}``, where
``{out}`` is the output directory, ``{sample}`` is the sample, and ``{ref}`` is
the name of the reference.

IDmut output file: Batch of relation vectors
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Each batch of relation vectors contains a ``IDmutBatchIO`` object and is saved
to the file ``idmut-batch-{num}.brickle``, where ``{num}`` is the batch number.
See :doc:`../../data/idmut/idmut` for details on the data structure.
See :doc:`../../formats/data/brickle` for more information on brickle files.

IDmut output file: Batch of read names
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Within each batch, the IDmut step assigns an index (a nonnegative integer) to
...
See :doc:`../../data/idmut/qnames` for details on the data structure.
See :doc:`../../formats/data/brickle` for more information on brickle files.

IDmut output file: Reference sequence
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

The IDmut step writes the reference sequence as a ``RefseqIO`` object to the
file ``refseq.brickle``.
See :doc:`../../data/idmut/refseq` for details on the data structure.
See :doc:`../../formats/data/brickle` for more information on brickle files.

IDmut output file: IDmut report
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

SEISMIC-RNA also writes a report file, ``idmut-report.json``, that records the
settings you used for running the IDmut step and summarizes the results.
See :doc:`../../formats/report/idmut` for more information.

IDmut: Troubleshoot and optimize
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If you encounted errors during the IDmut step, then the most likely cause is
that the FASTA file or settings you used for the IDmut step differ from those
...
- You ran ``seismic align`` and ``seismic idmut`` separately (instead of with
  ``seismic wf``), and you used a larger value for ``--min-reads`` during the
  IDmut step than the Align step.
  To check if this happened, open your report files from Align and IDmut and
  see if the field "Minimum number of reads in an alignment map" has a larger
  value in the IDmut report.
...
The solution for the problem is to ensure that you run ``seismic idmut`` with
...
IDmut crashes or hangs while producing few or no batch files
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
If so, then rerun IDmut with adjustments to one or both settings:
