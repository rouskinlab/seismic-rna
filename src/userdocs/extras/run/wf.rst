...
2.  run IDmut on the alignment maps to yield IDmut reports
3.  run Filter on the IDmut reports to yield Filter reports
4.  (optionally) run Cluster on the Filter reports to make Cluster reports
5.  run Table on the IDmut, Filter, and (optionally) Cluster reports to yield
    table files
6.  (optionally) run Fold on the Filter and (optionally) Cluster table files to
    yield predicted RNA secondary structure models
7.  (optionally) run Export on the IDmut, Filter, and (optionally) Cluster table
    files and (optionally) structure models to yield sample results
8.  run Graph on the IDmut, Filter, and (optionally) Cluster tables and
    (optionally) structure models to yield graph files

By contrast, for each Filter table file you input, SEISMIC-RNA will only
...
Align and IDmut steps; see :ref:`idmut_refs` for more information.
...
ways to list input files (see :doc:`../inputs`), for example ::
    ...
    seismic wf {refs.fa} sample1/refA.bam {out}/*/idmut/refB/idmut-report.json {out}/*/table/
...
any locations amid positional arguments, for example ::
    ...
    seismic wf {refs.fa} -x sample4/ -y sample5.fq.gz -z sample6.fq.gz sample1/refA.bam {out}/*/idmut/refB/idmut-report.json {out}/*/table/
